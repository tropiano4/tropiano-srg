 ...
 CONTAINS
	...
	FUNCTION ThreeJSymbols_get(j_1, j2, j_3)
		...
		ThreeJSymbols_get = TJ(j1 / 2, j3 / 2, (j2 - ABS(j1 - j3)) / 2)
                ...
                IF (ABS(ThreeJSymbols_get) .GT. 1.0) STOP "3J symbol greater than 1!"
		
		IF (ABS(ThreeJSymbols_get) .GT. 1.e-14) RETURN
...
END MODULE angmom
	...
	! I think this subroutine solves the equation Y - X = 0 using some kind of method
	! Dev_in is the initial value of Y - X
...
gsc:		DO
	...
	SUBROUTINE DiagonalizationMethod_get_MeanField(diagonal, S, Niter)		
		...
		TYPE (SymHartreeFockFieldProj) :: field1, field2
		TYPE (SymD3Tensor), DIMENSION(:), POINTER :: Ek_VAP
		...
		!TYPE (SymGenDensityGaugeProj), DIMENSION(:), POINTER :: C_Matrix, Deriv_Y
		TYPE (SymDensityProj), DIMENSION(:), POINTER :: C_Matrix, Deriv_Y
		TYPE (ProjectionCoeffs) :: CoeffsXY
...
!	integer la,u1,u2
		...
		IF (Niter .GE. 1) THEN
			...
			DO ta = 0, 1
				...
				CALL SymD3Tensor_new(Ek_VAP(ta))
				
				CALL SymEkTensorVAP_get_GammaProj(Ek_VAP(ta), EkField, genden_gauge, C_Matrix, Deriv_Y, CoeffsXY, ta, NGauge)
				
			END DO
			
			CALL SymD3Tensor_add(ek_tensor, Ek_VAP(0), Ek_VAP(1))
				
			write(*,'("Trace Ek (PROJECTION) = ",2f20.5)') SymD3Tensor_trace(ek_tensor)*factor
			...
			write(*,'("Trace Hpp (PROJECTION) - p = ",f20.5," n = ",f20.5)') SymD3Tensor_trace(HF_Delta%p(0)),&
											   SymD3Tensor_trace(HF_Delta%p(1)) 
			write(*,'("Trace Hph (PROJECTION) - p = ",f20.5," n = ",f20.5)') SymD3Tensor_trace(field1%p(0)),&
											   SymD3Tensor_trace(field1%p(1)) 
			...
			DO IndexGauge = 1, NGauge		
				...
				CALL SymHartreeFockFieldProj_del(C_Matrix(IndexGauge)%field%rho)
				CALL SymHartreeFockFieldProj_del(Deriv_Y(IndexGauge)%field%rho)
		...
		write(*,'("Trace Ek (NO PROJECTION) = ",2f20.5)') SymD3Tensor_trace(ek_tensor)
		...
		CALL SymHartreeFockFieldProj_add_SymD3Tensor(HF_Gamma, ek_tensor, field1)
			write(*,'("Trace Hph (NO PROJECTION) - p = ",f20.5," n = ",f20.5)') SymD3Tensor_trace(field2%p(0)),&
											   SymD3Tensor_trace(field2%p(1)) 
		...
		write(*,'("Trace Hpp (NO PROJECTION) - p = ",f20.5," n = ",f20.5)') SymD3Tensor_trace(HF_Delta%p(0)),&
										 SymD3Tensor_trace(HF_Delta%p(1)) 
...
END MODULE diagmeth
 ...
 CONTAINS
	...
	SUBROUTINE ProjectionPreparation(UV, CoeffsXY, genden_gauge, C_Matrix, Deriv_Y, NGauge)
		...
		TYPE (SymGenDensityHFProj) :: gendenhf, genden_C
		TYPE (SymHartreeFockFieldProj) :: HF
		TYPE (SymDensityProj), DIMENSION(:), POINTER :: C_Matrix, Deriv_Y
		...
		DO IndexGauge = 1, NGauge
			CALL SymHartreeFockBogolFieldProj_new(C_Matrix(IndexGauge)%field)
			CALL SymHartreeFockBogolFieldProj_new(Deriv_Y(IndexGauge)%field)
		END DO
		...
		! Transform the density into the proper form for later user
		
		!CALL SymDensity_new_GenDensityProj(density, genden)
		
		HF = gendenhf
			
		!CALL SymGenDensityHFProj_del(gendenhf)
		...
		DO ta = 0, 1
			...
			DO IndexGauge = 1, NGauge
				...
				DO a = 0, Lmax
										...
										d = MIN(Nmax, NmaxOfL(a))
					IF (Basis .EQ. 1 .OR. CompHO .EQ. 1) 	d = ((N_0 - a) / 2) + 1
					...
					ALLOCATE(U(Nmax, Nmax))
					ALLOCATE(rho(d, d))
					ALLOCATE(TempMat(d, d))
		
					U = SymD3Tensor_matrix(HF%p(ta), a)
					rho = U(1:d, 1:d)
					...
					C_Matrix(IndexGauge)%field%rho%p(ta)%d3tensor(a)%d2 = Phase*rho
					
		!write(*,'(/," a = ",i4, " Dimension d = ",i4," DIM = ",i4,/)') a, d, DIM(a)
		!DO ii = 1, d
		!	DO jj = 1, ii
		!		WRITE(*,'("i = ",i4," j = ",i4," C(i,j) = ",2F12.5," C*(j,i) = ",2F12.5)') &
		!			ii, jj, C_Matrix(IndexGauge)%field%rho%p(ta)%d3tensor(a)%d2(ii,jj), &
		!			CONJG(C_Matrix(IndexGauge)%field%rho%p(ta)%d3tensor(a)%d2(jj,ii))
		!	END DO
		!END DO
		...
		CALL SymGenDensityHFProj_copy(gendenhf, HF)
					
		CALL SymGenDensityHFProj_copy(genden_C, C_Matrix(IndexGauge)%field%rho)
		...
		DO ta = 0, 1
			...
			DO a = 0, 2*Lmax
				...
				genden_gauge(IndexGauge)%rho%rho(ta, a)%store = MATMUL(genden_C%rho(ta, a)%store, gendenhf%rho(ta, a)%store)
		...
		DO ta = 0, 1
			...
			DO a = 0, Lmax
				...
				DO IndexGauge = 1, NGauge
					...
					U = SymD3Tensor_matrix(C_Matrix(IndexGauge)%field%rho%p(ta), a)
					rho = U(1:d, 1:d)
					
					TempMat = TempMat + (1.0 - Phase)*CoeffsXY%y_l(IndexGauge, ta) * rho
				...
				DO IndexGauge = 1, NGauge
					...
					V = 0.5*CoeffsXY%y_l(IndexGauge, ta)*(1.0 - Phase)* rho - 0.5*CoeffsXY%y_l(IndexGauge, ta)*TempMat
										
					CALL SymD3Tensor_assign_Matrix(Deriv_Y(IndexGauge)%field%rho%p(ta), a, V)
	...
	SUBROUTINE SymEkTensorVAP_get_GammaProj(HF_out, EkTensor, genden_gauge, C_Matrix, Deriv_Y, CoeffsXY, ta, NGauge)
		...
		INTEGER, INTENT(IN) :: ta, NGauge
		
		TYPE (SymDensityProj) :: density_rho
		TYPE (SymD3Tensor) :: Tensor_0, Tensor_1, Tensor_2, Tensor_3, HF

		COMPLEX, DIMENSION(1:Nmax, 1:Nmax) :: M, MT
		
		COMPLEX :: phase
		DOUBLE PRECISION :: Trace, AngleGauge, pi
		INTEGER :: IndexGauge, a
		
		CALL SymD3Tensor_new(Tensor_0)
		CALL SymD3Tensor_new(Tensor_1)
		CALL SymD3Tensor_new(Tensor_2)
		CALL SymD3Tensor_new(Tensor_3)
		CALL SymD3Tensor_new(HF)

		pi = 4.0*ATAN(1.0)

		CALL SymHartreeFockBogolFieldProj_new(density_rho%field)	
		...
		DO IndexGauge = 1, NGauge
			...
			Trace = SymD3Tensor_product2(EkTensor, density_rho%field%rho%p(ta))
		
			CALL SymD3Tensor_product(Tensor_0, Trace*CMPLX(1,0), Deriv_Y(IndexGauge)%field%rho%p(ta))
		
			! First Term

			CALL SymD3Tensor_MatrixProduct(Tensor_1, EkTensor, C_Matrix(IndexGauge)%field%rho%p(ta))
			CALL SymD3Tensor_MatrixProduct(Tensor_2, density_rho%field%rho%p(ta), Tensor_1)

			phase = - 1.0 + CEXP(-CMPLX(0,1)*2.0*AngleGauge)

			CALL SymD3Tensor_product(Tensor_3, phase, Tensor_2)
		
			CALL SymD3Tensor_add(Tensor_2, Tensor_1, Tensor_3)
		
			CALL SymD3Tensor_product(Tensor_1, CoeffsXY%y_l(IndexGauge, ta), Tensor_2)
		
			CALL SymD3Tensor_add(HF, Tensor_0, Tensor_1)
			CALL SymD3Tensor_add(HF_out, HF_out, HF)
			
		END DO
	
		! Forcing the matrix of the kinetic energy to be hermitian
	
		CALL SymD3Tensor_assign1(HF, HF_out)
		
		DO a = 0, Lmax
			M =  SymD3Tensor_matrix(HF, a)
			MT = TRANSPOSE(CONJG(M))
			CALL SymD3Tensor_assign_Matrix(HF, a, MT)
		END DO
		
		CALL SymD3Tensor_add(Tensor_1, HF_out, HF)
		CALL SymD3Tensor_product(HF_out, 0.5*CMPLX(1,0), Tensor_1)
		
		CALL SymD3Tensor_del(HF)
		CALL SymD3Tensor_del(Tensor_0)
		CALL SymD3Tensor_del(Tensor_1)
		CALL SymD3Tensor_del(Tensor_2)
		CALL SymD3Tensor_del(Tensor_3)
...
MODULE gauss
 ...
 CONTAINS
	...
	SUBROUTINE GaussQuadrature_new(gauss, n)
		...
		ALLOCATE(gauss%w(0:n)) !TODO ¿El tamaño maximo es n?
	...
	SUBROUTINE gbtql2(n, d, e, z, ierr)
		...
		DOUBLE PRECISION b, c, f, g, p, r, s, test
...
!	USE matrix
	...
	TYPE Matrix2D
		COMPLEX, DIMENSION(:, :), POINTER :: value
	END TYPE
...
END MODULE genden
 ...
 CONTAINS
	...
	SUBROUTINE Global_new
		...
		WRITE(*,'(/,"--- Parameters of the Gogny force ---",/)')
	...
	FUNCTION IHFLS(na, nc, la, nb, nd, lb)
		...
		DO Ipoint = 1, Npoint
			Integrand(Ipoint) = WaveFun(Ipoint,IndexBraOne) *WaveFun(Ipoint,IndexBraTwo)* &
					...
					2.0*WaveFun(Ipoint,IndexKetOne) *WaveFun(Ipoint,IndexKetTwo)/RadMesh(Ipoint) ) &
					   /RadMesh(Ipoint)**3
		END DO
...
! Input parameters module
 MODULE input
	...
	INTEGER :: Nsize, Npoint, Lmax, Nmax, Isospin
        ...
        DOUBLE PRECISION :: Ecut = -1.0, Convergence = 1.e-3, anneal = 0.5
		...
		DO WHILE (keyword .NE. "FINISH")
			  ...
			  !	  the unity: We need more wave-functions (more n values) for this.
			 	 ...
			 	 ! Printing the characteristics of the basis with respect to quantum numbers
                                ...
                                WRITE(*,'(/,"--- Cut-off Energy  : Ecut = ",F10.3)') Emax                                
                                WRITE(*,'(/,"--- Maximum n value : ncut = ",I7)') Nmax
...
!TODO		CALL Nucleus_show_ExperimentalData(consistency%density%nucleus)
	...
	SUBROUTINE SymGammaTensorVAP_get_GammaDDPhi(HF_out, Mfactor, CoeffsXY, genden_gauge, C_Matrix, Deriv_Y, NGauge, IndexGauge, ta)
		...
		TYPE (ProjectionCoeffs), INTENT(IN) :: CoeffsXY
		TYPE (SymHartreeFockFieldProj), DIMENSION(:), POINTER :: Mfactor
		!TYPE (SymGenDensityGaugeProj), DIMENSION(:), POINTER :: genden_gauge, C_Matrix, Deriv_Y
		TYPE (SymGenDensityGaugeProj), DIMENSION(:), POINTER :: genden_gauge
		TYPE (SymDensityProj), DIMENSION(:), POINTER :: C_Matrix, Deriv_Y
		
		INTEGER, INTENT(IN) :: ta, IndexGauge, NGauge
		
		TYPE (SymHartreeFockFieldProj) :: field1, HF_1, HF_2
		...
		DO IndexGauge_tau_prime = 1, NGauge
			...
			!CALL SymDensity_new_GenDensityProj_only_rho(density_gauge, genden_gauge(IndexGauge_tau_prime))			
			!CALL SymDensity_new_GenDensityProj_only_rho(dY, Deriv_Y(IndexGauge_tau_prime))
							
			Trace_1 = SymHartreeFockFieldProj_product_iso(Mfactor(IndexGauge), density_gauge(IndexGauge_tau_prime)%field%rho, ta)
				
			CALL SymHartreeFockFieldProj_scalar_iso(field1, 0.5*Trace_1, Deriv_Y(IndexGauge_tau_prime)%field%rho, ta)
	...
	SUBROUTINE SymGammaTensorVAP_get_DeltaP(HF_Delta_P, CoeffsXY, C_Matrix, NGauge, Delta01)
		...
		TYPE (SymHartreeFockFieldProj) :: HF, field1
		
		TYPE (SymDensityProj), DIMENSION(:), POINTER :: C_Matrix
		...
		DO ta = 0, 1
			...
			DO IndexGauge_tau = 1, NGauge
				...
				CALL SymHartreeFockFieldProj_full_product_iso(HF, Delta01(IndexGauge_tau), C_Matrix(IndexGauge_tau)%field%rho, ta, ta)
		...
		!CALL SymHartreeFockFieldProj_assign(HF, HF_Delta_P)
		!CALL SymHartreeFockFieldProj_assign(field1, HF_Delta_P)
		
		!DO ta = 0, 1
		!	DO a = 0, Lmax
			
		!		M =  SymD3Tensor_matrix(HF%p(ta), a)
		!		MT = TRANSPOSE(M)
		!		CALL SymD3Tensor_assign_Matrix(HF%p(ta), a, MT)
				
		!		M =  SymD3Tensor_matrix(HF%a(ta), a)
		!		MT = TRANSPOSE(M)
		!		CALL SymD3Tensor_assign_Matrix(HF%a(ta), a, MT)
		!						
		!	END DO
		!END DO
		
		!CALL SymHartreeFockFieldProj_add(CMatrix, field1, HF)
		!CALL SymHartreeFockFieldProj_product(HF_Delta_P, 0.5*CMPLX(1.0), Cmatrix)
		
		!CALL SymHartreeFockFieldProj_del(CMatrix)
	...
	SUBROUTINE SymGammaTensorVAP_get_GammaP(HF_Gamma_P, CoeffsXY, genden_gauge, C_Matrix, Deriv_Y, NGauge, Gamma, nucleus)
		...
		TYPE (SymHartreeFockFieldProj) :: HF, HF_sum, field1, field2
		...
		DO ta = 0, 1
			...
			DO IndexGauge_tau = 1, NGauge
				...
				Trace_1 = SymHartreeFockFieldProj_product_iso(Gamma(isoCoupling, IndexGauge_tau), &
									      density_gauge(IndexGauge_tau)%field%rho, ta)
			
				CALL SymHartreeFockFieldProj_scalar_iso(field1, 0.5*Trace_1, Deriv_Y(IndexGauge_tau)%field%rho, ta)
				...
				DO IndexGauge_tau_prime = 1, NGauge
					...
					Trace_2 = SymHartreeFockFieldProj_product_iso(Gamma(isoCoupling, IndexGauge_tau_prime), &
												density_gauge(IndexGauge_tau)%field%rho, ta)
				...
				CALL SymHartreeFockFieldProj_scalar_iso(field1, 0.5*Sum, Deriv_Y(IndexGauge_tau)%field%rho, ta)
	...
	SUBROUTINE SymGammaTensorVAP_get_LambdaP(HF_Lambda_P, CoeffsXY, genden_gauge, C_Matrix, Deriv_Y, NGauge, Delta01, Delta10, nucleus)
		...
		TYPE (SymHartreeFockFieldProj) :: HF, field1, field2
	...
	SUBROUTINE SymGammaTensorVAP_get_GammaProj(genden_gauge, C_Matrix, isoCoupling, AngleGauge, IndexGauge, Gamma, GammaP)
		...
		TYPE (SymHartreeFockFieldProj) :: HF_tmp_1, HF_tmp_2, HF_out
		TYPE (SymDensityProj) :: density_rho
		...
		DO ta = 0, 1
			...
			CALL SymHartreeFockFieldProj_full_product_iso(HF_tmp_1, Gamma(isoCoupling, IndexGauge), C_Matrix(IndexGauge)%field%rho, ta, tb)
			CALL SymHartreeFockFieldProj_full_product_iso(HF_tmp_2, density_rho%field%rho, HF_tmp_1, ta, tb)
	...
	SUBROUTINE SymGammaTensorVAP_get_GammaDDProj(genden_gauge, C_Matrix, ta, AngleGauge, IndexGauge, Gamma, GammaP)
		...
		!TYPE (SymGenDensityGaugeProj), DIMENSION(:), POINTER :: genden_gauge, C_Matrix
		TYPE (SymGenDensityGaugeProj), DIMENSION(:), POINTER :: genden_gauge
		TYPE (SymDensityProj), DIMENSION(:), POINTER :: C_Matrix
		
		TYPE (SymHartreeFockFieldProj) :: CMatrix, rho_gauge, HF_tmp_1, HF_tmp_2, HF_out
		
		COMPLEX :: Phase
		
		Phase = - 1.0 + CEXP(-CMPLX(0,1)*2.0*AngleGauge)
		
		CALL SymHartreeFockFieldProj_new(CMatrix)
		CALL SymHartreeFockFieldProj_new(rho_gauge)
		
		!CALL SymGenDensityHFProj_assign2(CMatrix, C_Matrix(IndexGauge)%rho)
		CALL SymGenDensityHFProj_assign2(rho_gauge, genden_gauge(IndexGauge)%rho)

		CMatrix = C_Matrix(IndexGauge)%field%rho
		
		CALL SymHartreeFockFieldProj_new(HF_tmp_1)
		CALL SymHartreeFockFieldProj_new(HF_tmp_2)
		CALL SymHartreeFockFieldProj_new(HF_out)
		
		CALL SymHartreeFockFieldProj_full_product_iso(HF_tmp_1, Gamma, CMatrix, ta, ta)
	...
	SUBROUTINE SymGammaTensorVAP_get_LambdaProj(genden_gauge, C_Matrix, AngleGauge, IndexGauge, Delta01, Delta10, Lambda)
		...
		TYPE (SymHartreeFockFieldProj) :: HF_tmp_1, HF_tmp_2, HF_out
		TYPE (SymDensityProj) :: density_C, density_CT, density_kap01, density_kap10
		...
		!CALL SymDensity_new_GenDensityProj_only_rho(density_C, C_Matrix(IndexGauge))

		CALL SymHartreeFockBogolFieldProj_new(density_C%field)  ! C matrix
		CALL SymHartreeFockBogolFieldProj_new(density_CT%field) ! Transposed of C matrix
		
		density_C = C_Matrix(IndexGauge)
		
		! Tranpose the matrix C
		DO ta = 0, 1
			DO a = 0, Lmax
				M = SymD3Tensor_matrix(C_Matrix(IndexGauge)%field%rho%p(ta), a)
				MT = TRANSPOSE(M)
				CALL SymD3Tensor_assign_Matrix(density_CT%field%rho%p(ta), a, MT)
			END DO
		END DO
		...
		DO ta = 0, 1
			...
			CALL SymHartreeFockFieldProj_full_product_iso(HF_tmp_1, Delta01(IndexGauge), density_C%field%rho, ta, ta)
			CALL SymHartreeFockFieldProj_full_product_iso(HF_tmp_2, density_kap10%field%kap, HF_tmp_1, ta, ta)
		
			CALL SymHartreeFockFieldProj_full_product_iso(HF_tmp_1, Delta10(IndexGauge), density_CT%field%rho, ta, ta)
			CALL SymHartreeFockFieldProj_full_product_iso(HF_out, density_kap01%field%kap, HF_tmp_1, ta, ta)
		
			DO a = 0, Lmax
				M = -SymD3Tensor_matrix(HF_out%p(ta), a)
				MT = TRANSPOSE(M)
				CALL SymD3Tensor_assign_Matrix(HF_out%p(ta), a, MT)
			END DO
		...
		CALL SymHartreeFockBogolFieldProj_del(density_C%field)
		CALL SymHartreeFockBogolFieldProj_del(density_CT%field)
...
!------------------------------------------------------------------------------!
 ...
 MODULE symd3t_proj
	...
	IMPLICIT NONE
        ...
        ! SymD2Tensor points to a 2D array or complex numbers	
	TYPE SymD2Tensor
		COMPLEX, DIMENSION(:, :), POINTER :: d2
	END TYPE
 ...
 CONTAINS
	...
	FUNCTION SymD3Tensor_trace(t_in)
		...
		DO la = 0, Lmax
			...
			DO na = 1, d
				SymD3Tensor_trace = SymD3Tensor_trace + REAL(t_in%d3tensor(la)%d2(na, na))
			END DO
		END DO
		RETURN
	...
	FUNCTION SymD3Tensor_matrix(t_in, la)
		COMPLEX, DIMENSION(Nmax, Nmax) :: SymD3Tensor_matrix
	...
	! Fill in all elements of t_out with a single complex number R1
	...
	! Fill in all elements of t_out with same l-value with a given square matrix of complex number
	...
	! The next functions gets the Trace of a product of 2 SymD3Tensor

	FUNCTION SymD3Tensor_product2(t1_in, t2_in)
		COMPLEX SymD3Tensor_product2
		TYPE (SymD3Tensor), INTENT(IN) :: t1_in, t2_in
	...
	! The next functions gets the matrix product of 2 SymD3Tensor. The result is therefore
	! also a SymD3Tensor. More precisely: t1_in and t2_in are SymD3Tensor, i.e. a collection 
	! of (Lmax+1) small matrices of size d*d, where d = DIM(la). The matrix product is defined
	! as another collection of (Lmax+1) small matrices of size d*d, t_out, such thaT:
	!
	!         t_out(la) = Matrix_product[ t1_in(la), t2_in(la) ]
	! 
	! Additionally, since all matrices are hermitian (in fact symmetric since also real), we 
	! have further simplifications.

	SUBROUTINE SymD3Tensor_MatrixProduct(t_out, t1_in, t2_in)
		TYPE (SymD3Tensor), INTENT(OUT) :: t_out
		TYPE (SymD3Tensor), INTENT(IN) :: t1_in, t2_in

		COMPLEX, DIMENSION(:, :), ALLOCATABLE :: M1, M2, M
		
		COMPLEX :: sum1
		INTEGER :: u1, u2, u3, d, k

		DO u1 = 0, Lmax
								d = Min(Nmax, NmaxOfL(u1))
			IF (Basis .EQ. 1 .OR. CompHO .EQ. 1) 	d = ((N_0 - u1) / 2) + 1
		
			ALLOCATE(M1(1:Nmax, 1:Nmax))
			ALLOCATE(M2(1:Nmax, 1:Nmax))
			ALLOCATE(M (1:Nmax, 1:Nmax))
		
			M1 = SymD3Tensor_matrix(t1_in, u1)
			M2 = SymD3Tensor_matrix(t2_in, u1)
		...
		RETURN
	END SUBROUTINE SymD3Tensor_MatrixProduct
 ...
 CONTAINS
	...
	SUBROUTINE SymDensityProj_initialize(density)
		...
		DO ta = 0, 1
			...
			density%field%rho%p(ta) = CMPLX(0, 0)
			density%field%rho%a(ta) = CMPLX(0, 0)
			density%field%kap%p(ta) = CMPLX(0, 0)
			density%field%kap%a(ta) = CMPLX(0, 0)
...
END MODULE symden_proj
 ...
 CONTAINS
	...
	! New addition rule for one isospin only
	SUBROUTINE SymHartreeFockFieldProjIso_add(HF_out, HF1_in, HF2_in, ta)
		TYPE (SymHartreeFockFieldProj), INTENT(INOUT) :: HF_out
		TYPE (SymHartreeFockFieldProj), INTENT(IN) :: HF1_in, HF2_in
		INTEGER, INTENT(IN) :: ta

		CALL SymD3Tensor_add(HF_out%p(ta), HF1_in%p(ta), HF2_in%p(ta))
		CALL SymD3Tensor_add(HF_out%a(ta), HF1_in%a(ta), HF2_in%a(ta))
		
		HF_out%GaugeAngle(ta) = HF1_in%GaugeAngle(ta)
		
		RETURN
	END SUBROUTINE SymHartreeFockFieldProjIso_add
	...
	! Full Matrix product of SymHartreeFockFieldProj objects
	SUBROUTINE SymHartreeFockFieldProj_full_product_iso(HF_out, HF1_in, HF2_in, ta, tb)
		TYPE (SymHartreeFockFieldProj), INTENT(INOUT) :: HF_out
		TYPE (SymHartreeFockFieldProj), INTENT(IN) :: HF1_in, HF2_in
		INTEGER, INTENT(IN) :: ta, tb

		CALL SymD3Tensor_MatrixProduct(HF_out%p(ta), HF1_in%p(ta), HF2_in%p(tb))
		CALL SymD3Tensor_MatrixProduct(HF_out%a(ta), HF1_in%a(ta), HF2_in%a(tb))
		
		HF_out%GaugeAngle(ta) = HF1_in%GaugeAngle(ta)
		
		RETURN
	END SUBROUTINE SymHartreeFockFieldProj_full_product_iso

	SUBROUTINE SymHartreeFockFieldProj_scalar_iso(HT_out, R1, HF_in, ta)
		TYPE (SymHartreeFockFieldProj), INTENT(INOUT) :: HT_out
		...
		CALL SymD3Tensor_product(HT_out%p(ta), R1, HF_in%p(ta))
		CALL SymD3Tensor_product(HT_out%a(ta), R1, HF_in%a(ta))
		
		HT_out%GaugeAngle(0) = HF_in%GaugeAngle(0)
		
		RETURN
	END SUBROUTINE SymHartreeFockFieldProj_scalar_iso
	...
	! The result is a complex number which does NOT depend on the
	! isospin.

	FUNCTION SymHartreeFockFieldProj_product2(HF1_in, HF2_in)
		COMPLEX SymHartreeFockFieldProj_product2
		TYPE (SymHartreeFockFieldProj), INTENT(IN) :: HF1_in, HF2_in

		COMPLEX, DIMENSION(0:1) :: sum2
		INTEGER ta
	...
	! Used to make the contraction of a field of given isospin with
	! a density of the same isospin. The result is a complex number 
	! (which depends on the isospin).

	FUNCTION SymHartreeFockFieldProj_product_iso(HF1_in, HF2_in, ta)
		COMPLEX SymHartreeFockFieldProj_product_iso
		TYPE (SymHartreeFockFieldProj), INTENT(IN) :: HF1_in, HF2_in
...
END MODULE symfield_proj
 ...
 CONTAINS
	...
	SUBROUTINE SymGDDph_GammaVAP(HF_out, gDDph_phi, gDDph_proj, HF_in_1,  HF_in_2, Gauge, Flag)
		TYPE (SymHartreeFockFieldProj), INTENT(INOUT) :: HF_out
		TYPE (SymGDDph), INTENT(INOUT) :: gDDph_phi, gDDph_proj
		TYPE (SymHartreeFockFieldProj), INTENT(IN) :: HF_in_1,  HF_in_2
		DOUBLE PRECISION, INTENT(IN) :: Gauge
		...
		CASE(1)
			...
			! Calculate the new density for both isospin channels from the current wave-functions and fields (HF_in)
                        ...
                        CALL SymGDDph_make_DD(gDDph_phi, HF_in_1, Gauge)
                        CALL SymGDDph_make_DD(gDDph_proj, HF_in_2, Gauge)
			...
			DO ta = 0, 1
				...
				DO la = 0, Lmax
					...
					DO na = 1, namax
						DO nc = 1, na
							HF_out%p(ta)%d3tensor(la)%d2(na, nc) = SymGDDph_MfactorHO(gDDph_phi, gDDph_proj, &
														na - 1, nc - 1, la, ta, Flag)
							HF_out%a(ta)%d3tensor(la)%d2(na, nc) = 0.0
						END DO
					END DO
				END DO
				
				CALL SymHartreeFockFieldProj_setGauge(HF_out, HF_in_1%GaugeAngle(ta), ta)
		...
		CASE(2)
			...
			! Calculate the new density for both isospin channels from the current wave-functions and fields (HF_in)
                        ...
                        CALL Make_DenGenFun(gDDph_phi, HF_in_1, Gauge)
                        CALL Make_DenGenFun(gDDph_proj, HF_in_2, Gauge)
			...
			DO ta = 0, 1
				...
				DO la = 0, Lmax
					...
					DO na = 1, namax
						DO nc = 1, na
							HF_out%p(ta)%d3tensor(la)%d2(na, nc) = SymGDDph_MfactorWS(gDDph_phi, gDDph_proj, &
															na, nc, la, ta, Flag)
							HF_out%a(ta)%d3tensor(la)%d2(na, nc) = 0.0
						END DO
					END DO
		...
		END SELECT
		RETURN
	END SUBROUTINE SymGDDph_GammaVAP
...
!TODO			sumi =  sumi + EXP(LOG(GaussLQ%gauss%w(i)) + LOG(aLag(i)) * (p1 + la) + LOG(d3) * (ALPHA - 1) + LOG(d5))
	...
	FUNCTION SymGDDph_MfactorHO(gDDph_phi, gDDph_proj, na, nc, la, ta, Flag)
		DOUBLE PRECISION SymGDDph_MfactorHO
		TYPE (SymGDDph), INTENT(IN) :: gDDph_phi, gDDph_proj
		INTEGER, INTENT(IN) :: na, nc, la, ta, Flag
		...
		DO p1 = 0, p1max
			...
			DO i = 1, NLag
				...
				d1 = gDDph_phi%dLag(    ta, i)
				d2 = gDDph_phi%dLag(1 - ta, i)
				
				! d3 = total density at point ri (ri is a node for the Laguerre integration)
				
				d3 = gDDph_proj%dLag(ta, i)
				
				IF (d3 .LT. 0.0) STOP "ERROR! Fuera de rango"
				
				IF (Flag .EQ. 0) THEN  ! Standard density
					d5 = (1.0 + 0.5 * x0) * d3 * d3 - (x0 + 0.5) * d1 * (d3 - ALPHA * d2)
				ELSE  		    ! M-integrals
					d5 = (x0 + 0.5) * d1 * d2 * ALPHA
				END IF
		...
		SymGDDph_MfactorHO = Gogny_t0(Gogny) * 0.5 * I_SALPHA3 * sump1
		
		RETURN
	END FUNCTION SymGDDph_MfactorHO
...
!TODO			pows(i) = pows(i) * aLag(i)
	...
	!-----------------------------------------------------------------------!
	!									!
	!  This function computes the integrals M_{ij} as defined in the notes	!
	!  about the projection. We need to consider 2 types of densities, the	!
	!  gauge-dependent gDDph_phi and the projected density gDDph_proj. Both	!
	!  are needed to compute the integral.
	!									!
	!-----------------------------------------------------------------------!
       
	FUNCTION SymGDDph_MfactorWS(gDDph_phi, gDDph_proj, na, nc, la, ta, Flag)
		DOUBLE PRECISION SymGDDph_MfactorWS
		TYPE (SymGDDph), INTENT(IN) :: gDDph_phi, gDDph_proj
		...
		IndexKet = IndexVecNL(nc,la)
                ...
                IF (IndexBra .EQ. 0 .OR. IndexKet .EQ. 0) THEN
			SymGDDph_MfactorWS = 0.0
			RETURN
                END IF
		
		! Calculating the function to integrate at each point:
		!    rho_phi_{n} * rho_phi_{p} * rho_proj^{alpha-1} * R_{i}(r)*R_{j}(r)
					
	        ALLOCATE(Integrand(1:Npoint))
		
		DO i = 1, Npoint
			
			! d1 = gauge-dependent density for isospin  ta  at point ri
			! d2 = gauge-dependent density for isospin 1-ta at point ri
			
			d1 = gDDph_phi%dLag(    ta, i)
			d2 = gDDph_phi%dLag(1 - ta, i)
			
			! d3 = total projected density at point ri
				
			d3 = gDDph_proj%dLag(ta, i)
				
			IF (d3 .LT. 0.0) THEN
				WRITE(*,'("i = ",I4," d1 = ",F20.15," d2 = ",F20.15," d3 = ",F20.15)') i,d1,d2,d3
				STOP "ERROR - Negative density in SymGDDph_Mfactor"
			END IF
		...
		SymGDDph_MfactorWS = Gogny_t0(Gogny) * res
		
		RETURN
	END FUNCTION SymGDDph_MfactorWS
...
END MODULE symgdd_proj
 ...
 CONTAINS
	...
	SUBROUTINE SymGenDensityProj_make_Block(genden, ta, a, U, V)
		TYPE (SymGenDensityProj), INTENT(INOUT) :: genden
		COMPLEX, DIMENSION(:, :), INTENT(INOUT) :: U, V !TODO(INOUT?)
		INTEGER, INTENT(IN) :: ta, a

		COMPLEX, DIMENSION(:, :), ALLOCATABLE :: S0, S1, S2, S3
		COMPLEX :: U0, V0, Gauge
		...
		IF (genden%nucleus%is_blocking(ta) .AND. (a .EQ. genden%nucleus%ia(ta))) THEN
			...
			S1 = MATMUL(CONJG(V), TRANSPOSE(V))
			S3 = MATMUL(U, TRANSPOSE(CONJG(V)))
	...
	SUBROUTINE SymGenDensityProj_reduce(genden_out, genden_in, t_in)
		TYPE (SymGenDensityProj), INTENT(INOUT) :: genden_out
		TYPE (SymGenDensityProj), INTENT(IN) :: genden_in
		TYPE (SymD3Tensor), INTENT(IN) :: t_in

		DOUBLE PRECISION b
		TYPE (SymGenDensityHFProj) tmp

		b = Nucleus_get_b(genden_in%nucleus)
		
		CALL SymGenDensityHFProj_new(tmp)
		CALL SymGenDensityHFProj_reduce(tmp, genden_in%rho, t_in)
		
		CALL SymGenDensityProj_new_GammaDelta(genden_out, tmp, genden_in%kap, b)
		CALL SymGenDensityHFProj_del(tmp)
		
		RETURN
	END SUBROUTINE SymGenDensityProj_reduce
...
!----------------------------------------------------------------!
 ...
 CONTAINS
	...
	SUBROUTINE SymVBBph_calculate(vBBph, Lmin)
		TYPE (SymVBBph), INTENT(INOUT) :: vBBph
						...
						DO lb = 0, la
							...
							DO nb = 1, nbmax
								DO nd = 1, nb
									...
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v_local_same_part, la, na, nc, lb, nb, nd, sumi_sp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v_local_same_part, lb, nb, nd, la, na, nc, sumi_sp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v_local_diff_part, la, na, nc, lb, nb, nd, sumi_dp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v_local_diff_part, lb, nb, nd, la, na, nc, sumi_dp*CMPLX(1,0))
									...
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v1_exch_same_part, la, na, nc, lb, nb, nd, sum1_sp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v1_exch_same_part, lb, nb, nd, la, na, nc, sum1_sp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v1_exch_diff_part, la, na, nc, lb, nb, nd, sum1_dp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v1_exch_diff_part, lb, nb, nd, la, na, nc, sum1_dp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v2_exch_same_part, la, na, nc, lb, nb, nd, sum2_sp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v2_exch_same_part, lb, nb, nd, la, na, nc, sum2_sp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v2_exch_diff_part, la, na, nc, lb, nb, nd, sum2_dp*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBph%v2_exch_diff_part, lb, nb, nd, la, na, nc, sum2_dp*CMPLX(1,0))
								
									IF (file_error .EQ. 0) THEN
										WRITE (file_desc, FMT="(6I3,6E24.16)", IOSTAT=file_error) &
											la, na, nc, lb, nb, nd, REAL(sumi_sp), REAL(sumi_dp), &
										  REAL(sum1_sp), REAL(sum1_dp), REAL(sum2_sp), REAL(sum2_dp)
									END IF
								END DO
							END DO
						END DO
						...
						DO lb = 0, la
							...
							DO nb = 1, nbmax
								DO nd = 1, nb
									...
									DO i = 0, 1
										total_ibb = IBrinkBooker(na, la, nb, lb, nc, la, nd, lb, 0, mu(i), icount)
										IF (nb .NE. nd) THEN
											total_ibb = total_ibb + IBrinkBooker(na, la, nd, lb, nc, la, nb, lb, 0, mu(i), icount)
											total_ibb = total_ibb / 2.0
										END IF
										sumi_sp = sumi_sp + (coef_local_same_part(i) * total_ibb)
										sumi_dp = sumi_dp + (coef_local_diff_part(i) * total_ibb)
									...
									DO k = kmin, kmax, 2
										...
										cuad = CUAD2(la, lb, k)
										DO i = 0, 1
											total_ibb = IBrinkBooker(na, la, nb, lb, nd, lb, nc, la, k, mu(i), icount)
											IF(nb .NE. nd) THEN
												total_ibb = total_ibb + IBrinkBooker(na, la, nd, lb, nb, lb, nc, la, k, mu(i), icount)
												total_ibb = total_ibb / 2.0
											END IF
											sum1_sp = sum1_sp + (coef_exch1_same_part(i) * tres_j_cuad * total_ibb)
											sum1_dp = sum1_dp + (coef_exch1_diff_part(i) * tres_j_cuad * total_ibb)
	...
	SUBROUTINE SymVBBph_write(vBBph)
		...
		COMPLEX :: sumi_sp, sumi_dp, sum1_sp, sum1_dp, sum2_sp, sum2_dp
					...
					DO lb = 0, la
						...
						DO nb = 1, nbmax
							DO nd = 1, nb
								...
								IF (file_error .EQ. 0) THEN
									WRITE (file_desc, FMT="(6I3,6E24.16)", IOSTAT=file_error) &
									la, na, nc, lb, nb, nd, REAL(sumi_sp), REAL(sumi_dp), REAL(sum1_sp), &
												REAL(sum1_dp), REAL(sum2_sp), REAL(sum2_dp)
								END IF
							END DO
						END DO
					END DO
	...
	SUBROUTINE SymVBBpp_calculate(vBBpp, Lmin)
		TYPE (SymVBBpp), INTENT(INOUT) :: vBBpp
		INTEGER, INTENT(INOUT) :: Lmin
						...
						DO lb = 0, la
							...
							DO nb = 1, nbmax
								DO nd = 1, nb
									...
									CALL SymD3Tensor_SymD3Tensor_assign(vBBpp%v1_pair, la, na, nc, lb, nb, nd, sum1*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBpp%v1_pair, lb, nb, nd, la, na, nc, sum1*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBpp%v2_pair, la, na, nc, lb, nb, nd, sum2*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vBBpp%v2_pair, lb, nb, nd, la, na, nc, sum2*CMPLX(1,0))
									
									IF (file_error .EQ. 0) THEN
										WRITE (file_desc, FMT="(6I3,2E24.16)", IOSTAT=file_error) &
											la, na, nc, lb, nb, nd, REAL(sum1), REAL(sum2)
									END IF
							...
							DO nb = 1, nbmax
								DO nd = 1, nb
									...
									DO k = kmin, kmax, 2
										...
										DO i = 0, 1
											total_ibb = IBrinkBooker(na, la, nc, la, nb, lb, nd, lb, k, mu(i), icount)
											IF(nb .NE. nd) THEN
												total_ibb = total_ibb + IBrinkBooker(na, la, nc, la, nd, lb, nb, lb, k, mu(i), icount)
												total_ibb = total_ibb / 2.0
											END IF
											sum1 = sum1 + (coef_pair1(i) * tres_j_cuad * total_ibb)
											sum2 = sum2 + (coef_pair2(i) * cuad * tres_j_cuad * total_ibb)
...
!---------------------------------------------------------------------!
 ...
 CONTAINS
	...
	SUBROUTINE SymVCph_calculate(vCph, Lmin)
		TYPE (SymVCph), INTENT(INOUT) :: vCph
		INTEGER, INTENT(INOUT) :: Lmin

		INTEGER, PARAMETER :: file_desc = 16
		INTEGER :: la, namax, na, nc, lb, nbmax, nb, nd
		INTEGER :: k, kmin, kmax, Lold, file_error

		DOUBLE PRECISION :: total_iC, tres_j_cuad, cuad
		
		COMPLEX :: sumi, sum1, sum2
								...
								DO nd = 1, nb
									...
									IF (file_error .EQ. 0) THEN
										WRITE (file_desc, FMT="(6I3,3E24.16)", IOSTAT=file_error) &
											la, na, nc, lb, nb, nd, REAL(sumi), REAL(sum1), REAL(sum2)
									END IF
	...
	SUBROUTINE SymVCpp_calculate(vCpp, Lmin)
		TYPE (SymVCpp), INTENT(INOUT) :: vCpp
		INTEGER, INTENT(INOUT) :: Lmin
...
!----------------------------------------------------------------!
 ...
 CONTAINS
						...
						DO lb = 0, la
							...
							DO nb = 1, nbmax
								DO nd = 1, nb
									...
									CALL SymD3Tensor_SymD3Tensor_assign(vLSph%v12, la, na, nc, lb, nb, nd, sum1*CMPLX(1,0))
									CALL SymD3Tensor_SymD3Tensor_assign(vLSph%v21, la, na, nc, lb, nb, nd, sum2*CMPLX(1,0))
								
									IF (la .NE. lb) THEN
										! Ojo, estan invertidos los terminos
										CALL SymD3Tensor_SymD3Tensor_assign(vLSph%v12, lb, nb, nd, la, na, nc, sum2*CMPLX(1,0))
										CALL SymD3Tensor_SymD3Tensor_assign(vLSph%v21, lb, nb, nd, la, na, nc, sum1*CMPLX(1,0))
									END IF
	...
	SUBROUTINE SymVLSph_get_Gamma(HF_out, vLSph, HF_in, ta, tb, ProjectionOn)
		TYPE (SymHartreeFockFieldProj), INTENT(INOUT) :: HF_out
		...
		! Atencion: si (y0 .NE. 1), este factor cambia la fuerza.
		! Se pretende que la fuerza de spin-orbita sea dependiente del isosoin.
		factor = Gogny_W0(Gogny) * I_4PI * CMPLX(1,0)

		IF (ProjectionOn .EQ. 1) THEN
		
			IF (ta .EQ. tb) THEN
				CALL SymD3Tensor_product(tmp3, (1.0 + x0)*CMPLX(1,0), HF_in%a(ta))
			ELSE
				CALL SymD3Tensor_product(tmp3, x0*CMPLX(1,0), HF_in%a(ta))
			END IF
		
			CALL SymD3Tensor_SymD3Tensor_product(tmp1, vLSph%v12, tmp3)
			CALL SymD3Tensor_product(HF_out%p(tb), factor, tmp1)

			IF (ta .EQ. tb) THEN
				CALL SymD3Tensor_product(tmp3, (1.0 + x0)*CMPLX(1,0), HF_in%p(ta))
			ELSE
				CALL SymD3Tensor_product(tmp3, x0*CMPLX(1,0), HF_in%p(ta))
			END IF
		
			CALL SymD3Tensor_SymD3Tensor_product(tmp1, vLSph%v21, tmp3)
			CALL SymD3Tensor_product(HF_out%a(tb), factor, tmp1)
			
		ELSE
		
			CALL SymD3Tensor_product(tmp1, (1.0 + x0)*CMPLX(1,0), HF_in%a(ta))
			CALL SymD3Tensor_product(tmp2, x0*CMPLX(1,0), HF_in%a(1-ta))
			CALL SymD3Tensor_add(tmp3, tmp1, tmp2)
			
			CALL SymD3Tensor_SymD3Tensor_product(tmp1, vLSph%v12, tmp3)
			CALL SymD3Tensor_product(HF_out%p(tb), factor, tmp1)

			CALL SymD3Tensor_product(tmp1, (1.0 + x0)*CMPLX(1,0), HF_in%p(ta))
			CALL SymD3Tensor_product(tmp2, x0*CMPLX(1,0), HF_in%p(1-ta))
			CALL SymD3Tensor_add(tmp3, tmp1, tmp2)
						...
						DO lb = 0, la
							...
							DO nb = 1, nbmax
								DO nd = 1, nb
									...
									CALL SymD3Tensor_SymD3Tensor_assign(vLSpp%v22, la, na, nc, lb, nb, nd, sumi*CMPLX(1,0))
									
									IF (la .NE. lb) THEN
										CALL SymD3Tensor_SymD3Tensor_assign(vLSpp%v22, lb, nb, nd, la, na, nc, sumi*CMPLX(1,0))
									END IF
	...
	! Calculating the HF spin-orbit fields Gamma_LS

	SUBROUTINE SymVLSpp_get_Delta(HF_out, vLSpp, P_in, ta, tb)
		TYPE (SymHartreeFockFieldProj), INTENT(INOUT) :: HF_out
		...
		HF_out%p(tb) = CMPLX(0,0)

		factor = Gogny_W0(Gogny) * I_4PI * CMPLX(1,0)

		CALL SymD3Tensor_SymD3Tensor_product(tmp, vLSpp%v22, P_in%a(ta))
		CALL SymD3Tensor_product(HF_out%a(tb), factor, tmp)

		HF_out%GaugeAngle(tb) = P_in%GaugeAngle(ta)
		
		CALL SymD3Tensor_del(tmp)
...
!		INTEGER dimensionUV
 ...
 CONTAINS
	...
	SUBROUTINE WaveFunction_new(wave, N, Z)
		...
		CALL Nucleus_new(wave%nucleus, N, Z, b)
