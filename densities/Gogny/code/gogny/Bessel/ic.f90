...
!---------------------------------------------------------------------!
 ...
 MODULE ic
	...
	DOUBLE PRECISION, PARAMETER :: VC = 1.4399784085965135298D0 ! HFODD
	!DOUBLE PRECISION, PARAMETER :: VC = 1.44197028D0 ! Robledo
 ...
 CONTAINS
	...
	FUNCTION ICoulombHO(na, la, nb, lb, nc, lc, nd, ld, k)
		...
		sumN1 = SymKumar_get(na, la, nc, lc, N1max, k) * SumC(nb, lb, nd, ld, k, N1max, d2)

		DO N1 = N1max - 1, N1min, -1
			sumN1 = SymKumar_get(na, la, nc, lc, N1, k) * SumC(nb, lb, nd, ld, k, N1, d2) &
			      - (sumN1 * (DBLE(N1 + N2min + k) + 0.5D0) / DBLE(N1 + 1) / (DBLE(N1 + k) + 1.5D0) / 2.0D0)
		END DO
		...
		ICoulombHO = PI_COU * PAR(N1min + N2min) &
			...
			* (sumN1 * (d2 ** DBLE(-N1min - N2min - k))) / b_0
		RETURN
