...
gsc:		DO
			...
			IF (HFOnly .EQ. 0) THEN
				...
				CALL OneDimSolve_new(neutron_constrainer, diagonal%func, diagonal)
                                                                                       ...
                                                                                       ShiftReal = 0.0d0
                                 IF (IsoShiftLambda .EQ. 1 .OR. IsoShiftLambda .EQ. 2) ShiftReal = ShiftLambda
				diagonal%consistency%density%nucleus%lambda_np(1) = OneDimSolve_solve(neutron_constrainer, &
					DBLE(N)+ShiftReal, diagonal%consistency%density%nucleus%lambda_np(1), &
				...
				CALL OneDimSolve_new(proton_constrainer, diagonal%func, diagonal)
                                                                                       ...
                                                                                       ShiftReal = 0.0d0
                                 IF (IsoShiftLambda .EQ. 0 .OR. IsoShiftLambda .EQ. 2) ShiftReal = ShiftLambda

				diagonal%consistency%density%nucleus%lambda_np(0) = OneDimSolve_solve(proton_constrainer, &
					DBLE(Z)+ShiftReal, diagonal%consistency%density%nucleus%lambda_np(0), &
...
END MODULE gauss
 MODULE global
	...
	DOUBLE PRECISION, DIMENSION(0:1) :: m
	DATA m / 0.0241118636155835939, 0.0241118636155835939 /
	...
	DATA Gogny_t0 / 0.0, 1350.0, 1350.0 / ! D1S, D1, D1prime
...
MODULE symtalm
	...
	DOUBLE PRECISION, PARAMETER :: PI_B = 124.0251067211992807 !  = 4pi**2
