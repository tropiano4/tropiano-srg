 MODULE diagmeth
	...
	TYPE DiagonalizationMethod
		...
		TYPE (SymGenDensityProj) S ! Superhamiltonian
		TYPE (SymGenDensityProj) iterated ! Density at each iteration
		INTEGER ta
	END TYPE
...
gsc:		DO
			...
			CALL SymHartreeFockFieldProj_product(field2, 1.0D0-anneal, new_density%field%rho)
			CALL SymHartreeFockFieldProj_add(diagonal%consistency%density%field%rho, field2, field1)
			!CALL SymHartreeFockFieldProj_product(field1, anneal, diagonal%consistency%density%field%rho)
	...
	SUBROUTINE DiagonalizationMethod_get_MeanField(diagonal, S, Gauge)
		...
		! Protons
		CALL SymVLSph_get_Gamma(field1, diagonal%consistency%vLSph, diagonal%consistency%density%field%rho, 0, 0, 0)
		! Neutrons
		CALL SymVLSph_get_Gamma(field1, diagonal%consistency%vLSph, diagonal%consistency%density%field%rho, 1, 1, 0)
	...
	SUBROUTINE DiagonalizationMethod_ProjectedEnergy(diagonal)
		...
		DO ta = 0, 1
			...
			DO aa = 0, 2*Lmax
				...
				END DO
				np = np + DBLE(ja + 1) * REAL(trace,KIND=8)
				...
				trace = 0.0D0
				DO u1 = 1, d
					trace = trace + REAL(tmp(u1, u1),KIND=8)
				END DO
				R2 = R2 + DBLE(ja + 1) * trace !/ b2
