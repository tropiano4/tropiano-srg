...
CONTAINS
	...
	SUBROUTINE SymVCph_OpenFiles(Lmax, Nsmall)
		...
		IF (Nsmall < 10) THEN
			WRITE(fileout, "(A,I1,A)") "vC_N", Nsmall, "ph_WS.txt"
		ELSE
			WRITE(fileout, "(A,I2,A)") "vC_N", Nsmall, "ph_WS.txt"
		END IF
	...
	SUBROUTINE SymVCpp_OpenFiles(Lmax, Nsmall)
		INTEGER, INTENT(IN) :: Lmax, Nsmall
		INTEGER :: file_error
		...
		IF (Nsmall < 10) THEN
			WRITE(fileout, "(A,I1,A)") "vC_N", Nsmall, "pp_WS.txt"
		ELSE
			WRITE(fileout, "(A,I2,A)") "vC_N", Nsmall, "pp_WS.txt"
		END IF
