...
!----------------------------------------------------------------!
 ...
 CONTAINS
	...
	SUBROUTINE SymVBBph_OpenFiles(vBBph, Lmax, Nsmall)
		TYPE (SymVBBph), INTENT(INOUT) :: vBBph
		
		INTEGER, INTENT(IN) :: Lmax, Nsmall
		INTEGER :: file_error
		...
		IF (Nsmall < 10) THEN
			WRITE(fileout, "(A,I1,A)") "vBB_N", Nsmall, "ph_WS.txt"
		ELSE
			WRITE(fileout, "(A,I2,A)") "vBB_N", Nsmall, "ph_WS.txt"
		END IF
	...
	END SUBROUTINE SymVBBpp_new
 	...
 	SUBROUTINE SymVBBpp_OpenFiles(vBBpp, Lmax, Nsmall)
		TYPE (SymVBBpp), INTENT(INOUT) :: vBBpp
		...
		IF (Nsmall < 10) THEN
			WRITE(fileout, "(A,I1,A)") "vBB_N", Nsmall, "pp_WS.txt"
		ELSE
			WRITE(fileout, "(A,I2,A)") "vBB_N", Nsmall, "pp_WS.txt"
		END IF
