...
!	INTERFACE OPERATOR(*)
!		MODULE PROCEDURE SymVLSph_product, SymVLSpp_product
!	END INTERFACE
...
CONTAINS
	...
	SUBROUTINE SymVLSph_new(vLSph)
		...
		IF (N_0 < 10) THEN
			WRITE(vLSph%filename, "(A,I1,A)") "data/vLS", N_0, "ph.txt"
		ELSE
			WRITE(vLSph%filename, "(A,I2,A)") "data/vLS", N_0, "ph.txt"
		END IF
		RETURN
	...
	SUBROUTINE SymVLSph_calculate(vLSph)
		...
		IF (N_0 .LE. 16) THEN
			...
			OPEN(file_desc, FILE=vLSph%filename, ACTION="WRITE", IOSTAT=file_error)
			IF (file_error .NE. 0) THEN
				PRINT *, "*** AVISO: No se pueden escribir los resultados en ", vLSph%filename
			END IF
		END IF

		PRINT *, "Se van a calcular los elementos de matriz HF de LS"
		DO la = 0, N_0
			namax = ((N_0 - la) / 2) + 1
			DO na = 1, namax
				DO nc = 1, na
						...
						DO nb = 1, nbmax
							DO nd = 1, nb
								IF (lb .EQ. 0) THEN
									sum1 = DBLE(0.0)
								ELSE
									sum1 = DBLE(lb * (lb + 1)) * IHFLS(nb - 1, nd - 1, lb, na - 1, nc - 1, la)
								END IF
								IF (la .EQ. 0) THEN
									sum2 = DBLE(0.0)
								ELSE
									sum2 = DBLE(la * (la + 1)) * IHFLS(na - 1, nc - 1, la, nb - 1, nd - 1, lb)
								END IF
								...
								IF (la .NE. lb) THEN
									! Ojo, estan invertidos los términos
	...
	FUNCTION SymVLSph_read(vLSph)
		...
		OPEN(file_desc, FILE=vLSph%filename, ACTION="READ", IOSTAT=file_error)
		IF (file_error .NE. 0) THEN
			PRINT *, "No se pudo leer el archivo: ", vLSph%filename
			SymVLSph_read = .FALSE.
			RETURN
		END IF
...
!		PRINT *, "Leyendo:", vLSph%filename
						...
						DO nb = 1, nbmax
							DO nd = 1, nb
								...
								IF ((file_error .NE. 0) .OR. &
									...
									PRINT *, "Informacion no validad en el archivo: ", vLSph%filename
									CLOSE(file_desc)
									SymVLSph_read = .FALSE.
									RETURN
								END IF
	...
	SUBROUTINE SymVLSpp_new(vLSpp)
		...
		IF (N_0 < 10) THEN
			WRITE(vLSpp%filename, "(A,I1,A)") "data/vLS", N_0, "pp.txt"
		ELSE
			WRITE(vLSpp%filename, "(A,I2,A)") "data/vLS", N_0, "pp.txt"
		END IF
		RETURN
	...
	SUBROUTINE SymVLSpp_calculate(vLSpp)
		...
		IF (N_0 .LE. 16) THEN
			...
			OPEN(file_desc, FILE=vLSpp%filename, ACTION="WRITE", IOSTAT=file_error)
			IF (file_error .NE. 0) THEN
				PRINT *, "*** AVISO: No se pueden escribir los resultados en ", vLSpp%filename
			END IF
		END IF
							...
							DO nd = 1, nb
								...
								ELSE
									sumi = DBLE(PAR(la + lb) * 2.0 * la * (la + 1) * lb * (lb + 1)) * &
										IPLS(na - 1, nc - 1, la, nb - 1, nd - 1, lb)
								END IF
	...
	FUNCTION SymVLSpp_read(vLSpp)
		...
		OPEN(file_desc, FILE=vLSpp%filename, ACTION="READ", IOSTAT=file_error)
		IF (file_error .NE. 0) THEN
			PRINT *, "No se pudo leer el archivo: ", vLSpp%filename
			SymVLSpp_read = .FALSE.
			RETURN
		END IF
						...
						DO nb = 1, nbmax
							DO nd = 1, nb
								...
								IF ((file_error .NE. 0) .OR. &
									(la .NE. i1) .OR. (na .NE. i2) .OR. (nc .NE. i3) .OR. &
									(lb .NE. i4) .OR. (nb .NE. i5) .OR. (nd .NE. i6)) THEN
									PRINT *, "Informacion no validad en el archivo: ", vLSpp%filename
									CLOSE(file_desc)
									SymVLSpp_read = .FALSE.
									RETURN
								END IF
