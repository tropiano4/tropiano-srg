		...
		DO ta = 0, 1
			...
			DO a = 0, 2*Lmax
				...
				DO i = 1, dim_cur
					...
					SingleParticleP(ta)%value(dim_acc + i) = -(-1)**la * P(ta)%value(i, i)
					...
					averageDelta(ta) = averageDelta(ta) - P(ta)%value(i, i) * D(ta)%value(i) * (1.0 - D(ta)%value(i))
...
! Input parameters module
 MODULE input
	...
	INTEGER :: Nsize, Npoint, Lmax, Nmax, Isospin = 1

        DOUBLE PRECISION :: Ecut = -1.0, Convergence = 1.e-3, anneal = 0.5, facPair = 1.0, RadPoles = 1.0, ShiftLambda=0.0
 ...
 CONTAINS
	...
	SUBROUTINE Input_read
		...
		PRINT "(A,F8.5)", "Shift in Lagrange Parameter   : ", ShiftLambda
...
END MODULE r1r1
 ...
 CONTAINS
	...
	FUNCTION SelfConsistencyMethod_get_EkCM(consistency)
		...
		SelfConsistencyMethod_get_EkCM = (0.5 / A) * (consistency%density%field%rho * HF_Gamma)
	...
	FUNCTION SelfConsistencyMethod_get_LocalCoulombEnergy(consistency)
		...
		SelfConsistencyMethod_get_LocalCoulombEnergy = 0.5 * (consistency%density%field%rho * local_gamma)

		CALL SymHartreeFockField_del(local_gamma)
		RETURN
	END FUNCTION SelfConsistencyMethod_get_LocalCoulombEnergy

	! Energia de intercambio de Coulomb
	FUNCTION SelfConsistencyMethod_get_ExchangeCoulombEnergy(consistency)
		...
		SelfConsistencyMethod_get_ExchangeCoulombEnergy = 0.5 * (consistency%density%field%rho * exchange_gamma)
	...
	FUNCTION SelfConsistencyMethod_get_LSEnergy(consistency)
		...
		SelfConsistencyMethod_get_LSEnergy = 0.5* (consistency%density%field%rho * HF_Gamma)
	...
	FUNCTION SelfConsistencyMethod_get_EkPairing(consistency)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_EkPairing = 0.0 !0.5 * factor * (consistency%density%field%kap * EkCM_field)
		ELSE
			SelfConsistencyMethod_get_EkPairing = 0.0
		END IF
	...
	FUNCTION SelfConsistencyMethod_get_EkPairingIso(consistency,ta)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_EkPairingIso = 0.0 !0.5 * factor * (consistency%density%field%kap * EkCM_field)
		ELSE
			SelfConsistencyMethod_get_EkPairingIso = 0.0
		END IF
	...
	FUNCTION SelfConsistencyMethod_get_BBPairing(consistency)
		...
		ELSE
			SelfConsistencyMethod_get_BBPairing = 0.0
		END IF
	...
	FUNCTION SelfConsistencyMethod_get_BBPairingIso(consistency, ta)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_BBPairingIso = 0.5 * (consistency%density%field%kap * vbb_field)
		ELSE
			SelfConsistencyMethod_get_BBPairingIso = 0.0
		END IF
	...
	FUNCTION SelfConsistencyMethod_get_CoulombPairing(consistency)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_CoulombPairing = 0.0 !0.5 * (consistency%density%field%kap * delta)
		ELSE
			SelfConsistencyMethod_get_CoulombPairing = 0.0
		END IF
	...
	FUNCTION SelfConsistencyMethod_get_LSPairing(consistency)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_LSPairing = 0.0 !0.5 * (consistency%density%field%kap * delta)
		ELSE
			SelfConsistencyMethod_get_LSPairing = 0.0
		END IF
	...
	FUNCTION SelfConsistencyMethod_get_LSPairingIso(consistency, ta)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_LSPairingIso = 0.0 !0.5 * (consistency%density%field%kap * delta)
		ELSE
			SelfConsistencyMethod_get_LSPairingIso = 0.0
		END IF
 ...
 CONTAINS
	...
	FUNCTION SymD3Tensor_matrix(t_in, la)
		...
		!DOUBLE PRECISION, DIMENSION(((N_0 - la) / 2) + 1, ((N_0 - la) / 2) + 1) :: SymD3Tensor_matrix
                !DOUBLE PRECISION, DIMENSION(((Lmax - la) / 2) + 1, ((Lmax - la) / 2) + 1) :: SymD3Tensor_matrix
...
!TODO			pows(i) = pows(i) * aLag(i)
	...
	!									!
	!   Subroutine that calculates the density for each isospin channel	!
	!   taking as radial wave-functions arbitrary functions			!
	!									!
...
! matrices simetricas. Nota: el operador "()" corrige el error
	...
	SUBROUTINE SymKineticEnergy2Body_get_DeltaIso(HF_out, vEkCMpp, P_in, ta)
        ...
        HF_out%p(1-ta)=DBLE(0.0)
        HF_out%a(1-ta)=DBLE(0.0)
 ...
 CONTAINS
	...
	SUBROUTINE SymVBBpp_calculate(vBBpp, Read_BBpp, Lmin)
		...
		ELSE
			...
			write(*,'("After - Lmax = ",i4)') Lmax
			write(*,'("Test : ",E24.16)') vBBpp%v1_pair%d3tensor(0)%d2(1, 1)%d3tensor(0)%d2(1, 1)
