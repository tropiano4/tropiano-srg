...
! Input parameters module
 ...
 CONTAINS
	...
	SUBROUTINE Input_read
		...
		PRINT "(A,I3)",   "Index of the Wave-function    : ", IndexWave
		PRINT "(A,F8.3)", "Cut-off energy (if > 0)       : ", Ecut
...
END MODULE r1r1
 ...
 CONTAINS
	...
	FUNCTION SelfConsistencyMethod_get_EkPairing(consistency)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_EkPairing = 0.5 * factor * (consistency%density%field%kap * EkCM_field)
	...
	FUNCTION SelfConsistencyMethod_get_CoulombPairing(consistency)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_CoulombPairing = 0.5 * (consistency%density%field%kap * delta)
		ELSE
	...
	FUNCTION SelfConsistencyMethod_get_LSPairing(consistency)
		...
		IF (HFOnly .EQ. 0) THEN
			SelfConsistencyMethod_get_LSPairing = 0.5 * (consistency%density%field%kap * delta)
		ELSE
