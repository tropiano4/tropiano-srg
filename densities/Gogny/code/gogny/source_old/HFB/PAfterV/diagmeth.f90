...
gsc:		DO
	...
	SUBROUTINE DiagonalizationMethod_get_MeanField(diagonal, S, Gauge)
		...
		IF (HFOnly .EQ. 0) THEN
			...
			CALL SymHartreeFockFieldProj_product(ekcm_field, 1.0/A*CMPLX(1,0), field1)
		ELSE
			CALL SymKineticEnergy2Body_get_Delta(field1, diagonal%consistency%vEkCMpp, diagonal%consistency%density%field%kap, 0, 0, 0)
			CALL SymKineticEnergy2Body_get_Delta(field1, diagonal%consistency%vEkCMpp, diagonal%consistency%density%field%kap, 1, 1, 0)
			CALL SymHartreeFockFieldProj_product(ekcm_field, CMPLX(0, 0), field1)
		END IF
	...
	FUNCTION DiagonalizationMethod_operator(diagonal)
		...
		DO a = 0, 2*Lmax
			...
			xI = CMPLX(0,0)
			DO u1 = 1, d
				xI(u1, u1) = diagonal%func%x * CMPLX(1,0)
			END DO
	...
	SUBROUTINE DiagonalizationMethod_show_ParticleEnergies(diagonal, Gauge)
		...
		COMPLEX, DIMENSION(:, :), ALLOCATABLE :: S, V, R2Matrix, R2Initial
		...
		DO WHILE (REAL(SingleParticleE(1)%value(in(i))) .GE. -100.0 .AND. REAL(SingleParticleE(1)%value(in(i))) .LE. 40.0) 
			...
			END IF
					...
					i, spectr(Lvalue), Jvalue, REAL(Occup(1)%value(in(i))), REAL(SingleParticleE(1)%value(in(i)))
                        WRITE(file_unit_1,'(i4,3x,a8,f15.8)') i, label, REAL(SingleParticleE(1)%value(in(i)))
		...
		DO WHILE (REAL(SingleParticleE(0)%value(ip(i))) .GE. -100.0 .AND. REAL(SingleParticleE(0)%value(ip(i))) .LE. 40.0) 
			...
			END IF
					...
					i, spectr(Lvalue), Jvalue, REAL(Occup(0)%value(ip(i))), REAL(SingleParticleE(0)%value(ip(i)))
                        WRITE(file_unit_2,'(i4,3x,a8,f15.8)') i, label, REAL(SingleParticleE(0)%value(ip(i)))
			...
			DO a = 0, 2*Lmax
				...
				IF (Basis .EQ. 2) THEN
					...
					! mean-field potential)
               				CALL SymD3Tensor_product(ek_tensor, factor*CMPLX(1,0), EkField)
 ...
 1              	CONTINUE   
	...
	SUBROUTINE DiagonalizationMethod_ProjectedEnergy(diagonal)
		...
		TYPE (SymGenDensityGaugeProj), DIMENSION(:), POINTER :: genden_gauge_n, genden_gauge_p
		...
		COMPLEX, DIMENSION(:, :), ALLOCATABLE :: copyR2, copyRho, tmp, DensityRadial
		...
		ALLOCATE(genden_gauge_n(NGauge_n))
		
		DO IndexGauge = 1, NGauge_n
			CALL SymGenDensityHFProj_new(genden_gauge_n(IndexGauge)%rho)
			CALL SymGenDensityHFProj_new(genden_gauge_n(IndexGauge)%kap01)
			CALL SymGenDensityHFProj_new(genden_gauge_n(IndexGauge)%kap10)
		END DO
					
		ALLOCATE(genden_gauge_p(NGauge_p))
		
		DO IndexGauge = 1, NGauge_p
			CALL SymGenDensityHFProj_new(genden_gauge_p(IndexGauge)%rho)
			CALL SymGenDensityHFProj_new(genden_gauge_p(IndexGauge)%kap01)
			CALL SymGenDensityHFProj_new(genden_gauge_p(IndexGauge)%kap10)
		END DO
					
		! Calculate the gauge-dependent density
		
                CALL DiagonalizationMethod_set_Isospin(diagonal, 0)
		CALL SymGenDensityProj_make_DensityGauge(diagonal%UV, genden_gauge_p, diagonal%consistency%density%nucleus, &
													FactorProt, NGauge_p, 0)

                CALL DiagonalizationMethod_set_Isospin(diagonal, 1)
		CALL SymGenDensityProj_make_DensityGauge(diagonal%UV, genden_gauge_n, diagonal%consistency%density%nucleus, &
		...
		CALL SymGenDensityProj_make_DensityProj(genden_proj, CoeffsXY, genden_gauge_p, diagonal%UV, &
							diagonal%consistency%density%nucleus, FactorProt, NGauge_p, 0)
		CALL SymGenDensityProj_make_DensityProj(genden_proj, CoeffsXY, genden_gauge_n, diagonal%UV, &
		...
		IF (Basis .EQ. 1) THEN
			CALL SymGDDph_make_DD(gDDph, density%field%rho, Gauge)
		ELSE
			CALL Make_DenGenFun(gDDph, density%field%rho, Gauge)
		END IF
		
		ProjectedDD = SymGDDph_get_edd(gDDph)
		...
		DO IndexGauge = 1, NGauge_p
			...
			CALL SymDensity_new_GenDensityProj10(density10, genden_gauge_p(IndexGauge))
			...
			ProjectedEkCM = ProjectedEkCM + (0.5 / A) * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVBB_local = ProjectedVBB_local + 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVBB_exch = ProjectedVBB_exch + 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVC_local = ProjectedVC_local + 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVC_exch = ProjectedVC_exch + 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVLS = ProjectedVLS + 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
		
			!
			! Pairing Channel
			!
			
			CALL SymDensity_new_GenDensityProj01(density01, genden_gauge_p(IndexGauge))
			
			!density01 = diagonal%consistency%density
			
                        ! Center of Mass Pairing
                        CALL SymKineticEnergy2Body_get_Delta(HF_Delta, diagonal%consistency%vEkCMpp, density10%field%kap, 0, 0, ProjectionOn)

                        P(0) =  SymHartreeFockFieldProj_product_iso(density01%field%kap, HF_Delta, 0)

                        ProjectedEkCM_Pair = ProjectedEkCM_Pair - ( 0.5/A ) * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVBB_Pair = ProjectedVBB_Pair - 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVC_Pair = ProjectedVC_Pair - 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
			...
			ProjectedVLS_Pair = ProjectedVLS_Pair - 0.5 * CoeffsXY%y_l(0)%index(IndexGauge)*P(0)
		...
		DO IndexGauge = 1, NGauge_n
			...
			CALL SymDensity_new_GenDensityProj10(density10, genden_gauge_n(IndexGauge))
			...
			ProjectedEkCM = ProjectedEkCM + (0.5 / A) * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVBB_local = ProjectedVBB_local + 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVBB_exch = ProjectedVBB_exch + 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVC_local = ProjectedVC_local + 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVC_exch = ProjectedVC_exch + 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVLS = ProjectedVLS + 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
		
			!
			! Pairing Channel
			!
			
			CALL SymDensity_new_GenDensityProj01(density01, genden_gauge_n(IndexGauge))
			
			!density01 = diagonal%consistency%density
			
                        ! Center of Mass Pairing
                        CALL SymKineticEnergy2Body_get_Delta(HF_Delta, diagonal%consistency%vEkCMpp, density10%field%kap, 1, 1, ProjectionOn)
			
			P(1) = SymHartreeFockFieldProj_product_iso(density01%field%kap, HF_Delta, 1)
		
			ProjectedEkCM_Pair = ProjectedEkCM_Pair - ( 0.5/A ) * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVBB_Pair = ProjectedVBB_Pair - 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVC_Pair = ProjectedVC_Pair - 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
			...
			ProjectedVLS_Pair = ProjectedVLS_Pair - 0.5 * CoeffsXY%y_l(1)%index(IndexGauge)*P(1)
		...
		DO ta = 0, 1
			...
			DO aa = 0, 2*Lmax
				...
				trace = 0.0
				DO u1 = 1, d
					trace = trace + genden_proj%rho%rho(ta, aa)%store(u1, u1)
				END DO
				...
				! crash when using -C option at compilation stage
				copyR2 = SymD3Tensor_matrix(R2Field, la)
				copyRho = genden_proj%rho%rho(ta, aa)%store
		...
		IF (Basis .EQ. 2) THEN
					...
					DO ipoint = 1, Npoint
						...
						DO mm = 1, d
							...
							DO nn = 1, d
								...
								DensityRadial(ta, ipoint) = DensityRadial(ta, ipoint) &
												+ (ja + 1.0) * genden_proj%rho%rho(ta, aa)%store(mm, nn) &
												* WaveFun(ipoint,IndexBasis) &
												* WaveFun(ipoint,IndexKet)
			...
			DO ipoint = 0, Npoint
				WRITE(file_unit_1,'(5f20.16)') RadMesh(ipoint),DensityRadial(0, ipoint),DensityRadial(1, ipoint)
			END DO
