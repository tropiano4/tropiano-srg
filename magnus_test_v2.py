...
class Magnus_test(object):
    ...
    def __init__(self, k_array, H_initial, generator, k_magnus, ds, 
                 lambda_bd=0.00, coupled_channel=True):
        ...
        # Set-up for block-diagonal generator
        if generator == 'Block-diag':
            ...
            if coupled_channel:
                o = np.zeros((n,n))
                self.P = np.vstack( ( np.hstack( (p, o) ), 
                                      np.hstack( (o, p) ) ) )
                self.Q = np.vstack( ( np.hstack( (q, o) ), 
                                      np.hstack( (o, q) ) ) )
    ...
    def derivative(self, O_evolved):
        ...
        if self.generator == 'Wegner':
            ...
            G = self.P @ H_evolved @ self.P + self.Q @ H_evolved @ self.Q
...
if __name__ == '__main__':
    ...
    for generator in generator_list:
        ...
        if generator == 'Block-diag':
            magnus = Magnus_test(k_array, H_initial, generator, k_magnus, ds, 
                                 lambda_bd=lamb)
        ...
        ax1.loglog(d['s_array'], d['eta_array'], color=curve_color, 
                   linestyle=curve_style, label=generator_label)
    ...
    file_name = 'eta_omega_norms_kvnn_%d_generators_%s_%s_%s_k_magnus_%d' % \
                (kvnn, channel, generator_list[0], generator_list[1], k_magnus)
