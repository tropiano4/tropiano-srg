...
@kvnns = (01,02,03,04,05,07); # Paris, Bonn, reid93, nij1, nij2, cdbonn (in order)
...
foreach $kvnn (@kvnns) {
  ...
  foreach $lambda (@lambdas) {
    ...
    $ntot_print = sprintf("%d",$ntot);
    $output_dir_string = sprintf("vsrg_kvnn_%02d_lam%02.1f_kmax%s_kmid%s_ntot%s", $kvnn, $lambda, $kmax_print, $kmid_print, $ntot_print);
