...
#------------------------------------------------------------------------
#  File: run_generate_vsrg_vlowk_all.pl
#
...
$kmax = 15.0; # kmax ==> maximum momentum for bare potential
...
foreach $kvnn (@kvnns) {
  ...
  foreach $lambda (@lambdas) {
    ...
    $location = "";
    $kmax_print = sprintf("%2.0f",$kmax);
    $kmid_print = sprintf("%1.0f",$kmid);
    $output_dir_string = sprintf("vsrg_EKM_kvnn_%02d_lam%02.1f_kmax%s_kmid%s", $kvnn, $lambda, $kmax_print, $kmid_print);
    ...
    foreach $pw (@pws) {
      ...
      `./generate_vsrg_vlowk.x $input_filename $channel_filename`;
