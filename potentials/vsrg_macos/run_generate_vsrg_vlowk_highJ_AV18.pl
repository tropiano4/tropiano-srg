...
#------------------------------------------------------------------------
#  File: run_generate_vsrg_vlowk_highJ_AV18.pl
#
...
$kmax = 30.0; # kmax ==> maximum momentum for bare potential
#$kmax = 35.0; # kmax ==> maximum momentum for bare potential
$kmid = 3.0;  # mesh goes from 0 to kmid, then kmid to kmax
...
# The following list is of all the new 2017 Entem-Machleidt potentials
@kvnns = (70,71,72,73,74,75,76,77,78,79,80,81,82,83,84);
@kvnns = (6);
...
foreach $kvnn (@kvnns) {
  ...
  foreach $lambda (@lambdas) {
    ...
    $output_dir_string = sprintf("vsrg_newAV18_kvnn_%02d_lam%02.1f_kmax%s_kmid%s", $kvnn, $lambda, $kmax_print, $kmid_print);
