...
! AT: Updated 06/13/18
! The potential matrix elements are calculated mostly by chiralold. For AT's
! project, we need V(k,k') at LO.

! CUTOFF: Change cutoff of lambda in CUTOFN(1,1) for LO. This was originally
! set to 0.4 GeV.

! CONTACT TERMS: The LECs are given in the tables NLO, NNLO, etc within the
! chiralold routine. Need to fit the C_3S1^~ term to correspond to new
! cutoff value and set C_3S1 to zero. These are the fourth and third terms in
! the NLO table respectively.

! REGULATOR: The regulator is given by CUTFAC where p/Lambda (QQPUN1 is p) and
! p'/Lambda (QQPUN2 is p') to some power CUTPOT (i.e., CUTPOT = 2 corresponds
! to a Gaussian) in chiralold.

! PION EXCHANGE: For LO, we need to turn off the two-pion exchange terms.
! These terms are given by WTEN1S,WSIG1S,WC1T in chiralold.

! Spectral function regularization only affects multi-pion exchanges. At LO,
! this can be ignored.
...
! AT: A. Nogga et al. (2005) set CUTPOT = 4. Use this to reproduce files from Wendt (2011) paper.
! Original value: CUTPOT = 6.0_DPP

! A. Nogga et al. (2005) use g_A = 1.26. E. Epelbaum uses g_A = 1.29.
...
! CHANGES MADE BY AT: Fourth value in NLO corresponds to C_3S1^~ and must be fit for LO calculation
! Original value C_3S1~ = -0.181194189495330588 [10^4 GeV^-2]
! Here we fit C_3S1^~ for eps_d = -2.2245 MeV
! Lambda = 1.0 fm^-1: C_3S1^~ = -0.30592
! Lambda = 2.0 fm^-1: C_3S1^~ = -0.122963
! Lambda = 3.0 fm^-1: C_3S1^~ = -0.04743
! Lambda = 4.0 fm^-1: C_3S1^~ = 0.043292
! Lambda = 5.0 fm^-1: C_3S1^~ = 0.426
! Lambda = 6.0 fm^-1: C_3S1^~ = -0.45933
! Lambda = 7.0 fm^-1: C_3S1^~ = -0.203045
! Lambda = 8.0 fm^-1: C_3S1^~ = -0.135811
! Lambda = 9.0 fm^-1: C_3S1^~ = -0.0984445 (for CUTPOT = 4) DEFAULT
! Lambda = 9.0 fm^-1: C_3S1^~ = -0.0984445 (for CUTPOT = 2)
! Lambda = 10.0 fm^-1: C_3S1^~ = -0.069891
! Lambda = 11.0 fm^-1: C_3S1^~ = -0.042741
! Lambda = 12.0 fm^-1: C_3S1^~ = -0.0113085
! Lambda = 13.0 fm^-1: C_3S1^~ = 0.034059
! Lambda = 14.0 fm^-1: C_3S1^~ = 0.12402
! Lambda = 15.0 fm^-1: C_3S1^~ = 0.49613
! Lambda = 16.0 fm^-1: C_3S1^~ = -0.8686
! Lambda = 17.0 fm^-1: C_3S1^~ = -0.30281
! Lambda = 18.0 fm^-1: C_3S1^~ = -0.199585
! Lambda = 19.0 fm^-1: C_3S1^~ = -0.1532508
! Lambda = 20.0 fm^-1: C_3S1^~ = -0.125106
...
! AT: Change first value of CUTOFN to desired value of lambda. Second cutoff doesn't affect LO calculation.
! Original value: CUTOFN(1,1) = 0.40_DP
! Lambda = 1.0 fm^-1 -> 0.19732697 GeV
! Lambda = 2.0 fm^-1 -> 0.39465394 GeV
! Lambda = 3.0 fm^-1 -> 0.59198091 GeV
! Lambda = 4.0 fm^-1 -> 0.78930788 GeV
! Lambda = 5.0 fm^-1 -> 0.98663485 GeV
! Lambda = 6.0 fm^-1 -> 1.18396182 GeV
! Lambda = 7.0 fm^-1 -> 1.38128879 GeV
! Lambda = 8.0 fm^-1 -> 1.57861576 GeV
! Lambda = 9.0 fm^-1 -> 1.77594273 GeV
! Lambda = 10.0 fm^-1 -> 1.9732697 GeV
! Lambda = 11.0 fm^-1 -> 2.17059667 GeV
! Lambda = 12.0 fm^-1 -> 2.36792364 GeV
! Lambda = 13.0 fm^-1 -> 2.56525061 GeV
! Lambda = 14.0 fm^-1 -> 2.76257758 GeV
! Lambda = 15.0 fm^-1 -> 2.95990455 GeV
! Lambda = 16.0 fm^-1 -> 3.15723152 GeV
! Lambda = 17.0 fm^-1 -> 3.35455849 GeV
! Lambda = 18.0 fm^-1 -> 3.55188546 GeV
! Lambda = 19.0 fm^-1 -> 3.74921243 GeV
! Lambda = 20.0 fm^-1 -> 3.9465394 GeV
...
! AT: Gauss Legendre: ZPUN is the angle for dcos integral, ZWEI is the weight, and NZPUN is the number of points
CALL GAULEGMOD(-1.0_DP,1.0_DP,ZPUN,ZWEI,NZPUN)

! AT: Non-local regulator. CUTPOT controls the power in the exp (i.e., CUTPOT = 2 would give a Gaussian regulator).
! To match Wendt files, set CUTPOT = 4
...
DO I=1,NZPUN
  ...
  ! AT: LO terms in pion-exchange piece
...
! AT: Do not integrate NLO terms, set integrals to zero below (comment out the three calls below)
...
ELSE
  IF (JMOM == 1) THEN
    IF (OSTAT .NE. 2) THEN
       ! CHANGES MADE BY AT: Set NLO CC's to zero to make LO calc only
       ! Only non-zero CC should be CC(4) which is C_3S1^~
      !PPOT(4)=0.0*CC(3)*(QQPUN1**2+QQPUN2**2)+CC(4)
      !PPOT(1)=0.0*CC(5)*QQPUN1*QQPUN2
      !PPOT(2)=0.0*CC(6)*QQPUN1*QQPUN2
      !PPOT(5)=0.0*CC(9)*QQPUN1**2
      !PPOT(6)=0.0*CC(9)*QQPUN2**2
...
!PPOT=0.0_DP
! RTEN1S, RSIG1S, and RC1T are set to zero
! POTT corresponds to the triplet and POTS to the singlet
IF (FORCE .EQ. 'np') THEN
...
! Modified by AT: To make potential in units MeV^-2 as in smschiral_RS.f90,
! multiply by 2/pi * 10^6
! This is already accounted for in allvnnmodels_OPE???
!POTENT=POTENT*CUTFAC/(8.0_DP*PI**3)
