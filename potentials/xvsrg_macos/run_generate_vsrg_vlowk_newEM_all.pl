...
#------------------------------------------------------------------------
#  File: run_generate_vsrg_vlowk_new_EMall.pl
#
...
$kmax = 8.0; # kmax ==> maximum momentum for bare potential
...
foreach $kvnn (@kvnns) {
  ...
  foreach $lambda (@lambdas) {
    ...
    $output_dir_string = sprintf("vsrg_newEM_kvnn_%02d_lam%02.1f_kmax%s_kmid%s", $kvnn, $lambda, $kmax_print, $kmid_print);
