...
%     06/13/19 --- Added comments for editing purposes.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Maybe go with SRG first then mention IMSRG as an extension in solving the many-body problem.
\noindent{%
-- In recent years \textit{ab initio} methods have made strides in nuclear structure and reactions.
}
\\
...
\\
% Replace the bullet below. See IMSRG term paper.
...
\noindent{%
-- The IMSRG procedure requires solving a system of nonlinear, coupled differential equations which can often be stiff necessitating a high-order ODE solver.
}
\\
...
\noindent{%
-- A variant of the IMSRG that utilizes the Magnus expansion can overcome these limitations.
}
\\
...
\noindent{%
-- We test the Magnus implementation on chiral non-local NN potentials in free-space for several EFT cutoffs.
}
\\
...
\\
% Need transition sentence here.

\noindent{%
-- The past decade has seen a tremendous increase in the number of chiral NN potentials.
}
\\
...
\\
% Keep the last bullet in mind for the focus of this paper.
...
\noindent{%
\textbf{Magnus expansion formalism}
}
\\
...
\noindent{%
-- First, we compare Magnus to SRG evolution for large cutoff potentials with $G=H_D$ and $G=T_{rel}$.
}
\\
...
%
\noindent{%
-- Furthermore, it is dependent on the momentum mesh on which the potential is generated.
}
\\
...
%
\noindent{%
-- Figures \ref{fig:evolved_diags_Wendt_Lamb9_Wegner} and \ref{fig:evolved_diags_Wendt_Lamb9_T} show a more quantitative depiction of the evolved potentials.
}
\\
...
\\
% Make sure it's clear that the distributions are not unitarily equivalent.
...
% Re-do the figure below with larger fonts. Can also split into LaTeX sub-plots (a) and (b).
\\
\begin{figure}
  	\captionsetup{singlelinecheck=false,justification=raggedright}
  	\centering
...
%
\noindent{%
-- RMS radius and quadrupole moment of deuteron.
}
\\
...
\end{itemize}
%
\textbf{Evolution of operators} % Ask DF about what to include here.
%
\begin{figure}[H]
  	\captionsetup{singlelinecheck=false,justification=raggedright}
  	\centering
