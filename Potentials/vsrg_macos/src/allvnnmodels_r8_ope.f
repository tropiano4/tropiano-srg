...
! 
! BEGIN: ADDED by RJF on 18-DEC-2014   NEW addition on 30-Jul-2015 of ostat=-1
!  More kvnn numbers to accommodate new local chiral EFT from Epelbaum et al.
!         15 ichiral.f90 potential 
!    kvnn = 35-39  ostat=-1; cutnum=1-5  unregulated and nonlocal regulated OPE
...
!  where
!     ostat = -1 --> OPE unregulated  cutnum = 1,2
!     ostat = -1 --> OPE nonlocal cutnum = 3,4,5 --> nonlocal power 1,2,3, 500 MeV
!     ostat = 0,1,2,3,4 --> LO, NLO, N2LO, N3LO, N4LO  ! LO is OPE only
...
!
      IF (kvnn.eq.15) THEN
        ...
        cutnum = 2  ! default
      ENDIF
      IF ((kvnn.ge.35).and.(kvnn.le.39)) THEN 
        ostat = -1   ! LO
        cutnum = kvnn - 34  ! check in chart above
