! Modified 30-July-2015 by Dick Furnstahl (furnstahl.1@osu.edu) so that LO has only OPE.
!   This is OSTAT = 0.  Also added OSTAT = -1 for non-regulated OPE.
!   Changes marked by [rjf]
!**************************************************************************************************
!**************************************************************************************************
!**************************************************************************************************
!****                                                                                          ****
...
!****             E.Epelbaum, H.Krebs, U.-G.Meißner, arXiv:1412.0142[nucl-th],                 ****
...
IF (OSTAT .EQ. -1) THEN   ! [rjf] OPE unregulated
  POTENLONG(2,2)=POTLONGCOORD(4)
  POTENLONG(3,2)=POTLONGCOORD(3)
  POTENLONG(2,3)=4D0*(POTLONGCOORD(6)-POTLONGCOORD(4))
  POTENLONG(3,3)=4D0*(POTLONGCOORD(5)-POTLONGCOORD(3))
END IF  

IF (OSTAT .GE. 0) THEN   ! [rjf] OPE regulated
...
IMPLICIT NONE
INTEGER                    ::I,J,OSTAT,CUTNUMR,JMOM,NEXP   ! [rjf]
REAL(KIND(0.0D0))          ::PPUN1,PPUN2,POTEN(6),POTENOPERATOR(NSPIN,NISO),Q,&
                             ...
                             POTISOT(6),POTISOS(6),POTENCONT(6),LAMBDAMOM
...
CASE ('np')
  DO I=1,NZPUN
    Q=SQRT(PPUN1**2+PPUN2**2-2D0*PPUN1*PPUN2*ZPUN(I))
    CALL CHIRALMOMLR(OSTAT,Q,CUTNUMR,POTENOPERATOR)
    ! [rjf] Put the nonlocal regulator here?
    IF (OSTAT .EQ. -1) THEN
      LAMBDAMOM = 0.50D0  ! [rjf] 500 MeV = 0.5 GeV
      SELECT CASE (CUTNUMR)
        CASE (3)
          NEXP = 1;
          POTENOPERATOR = POTENOPERATOR * EXP(-((PPUN1**2+PPUN2**2)/LAMBDAMOM**2)**NEXP)
        CASE (4)
          NEXP = 2;
          POTENOPERATOR = POTENOPERATOR * EXP(-((PPUN1**2+PPUN2**2)/LAMBDAMOM**2)**NEXP)
        CASE (5)
...
IF (OSTAT .GE. 0) THEN
  ...
  SELECT CASE (OSTAT)
    CASE (0)
    WRITE(*,*)'   Improved chiral NN potential; OPE only'  ! [rjf]
    CASE (1)
...
ELSE IF (OSTAT .EQ. -1) THEN   ! [rjf]
  SELECT CASE (CUTNUMR)
    ...
    WRITE(*,*)'   unregulated'
    CASE (3)
    WRITE(*,*)'   nonlocal, nexp=1, 500 MeV'
    CASE (4)
    WRITE(*,*)'   nonlocal, nexp=2, 500 MeV'
    CASE (5)
    WRITE(*,*)'   nonlocal, nexp=3, 500 MeV'
    CASE DEFAULT
    WRITE(*,*)'Wrong value of CUTNUMR'
    STOP
END SELECT
...
SELECT CASE (OSTAT)
CASE (-1)    ! [rjf] CASE(-1) added for no regulator
  DO I=1,4
    CC(I)=0.0D0 * CLO(I,CUTNUMR)*FAKT  ! [rjf] set to zero
  END DO
  SELECT CASE (JMOM)
  CASE (0)
    SELECT CASE (FORCE)
