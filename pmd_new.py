...
class pair_momentum_distributions(object):
    ...
    def theta_1_deltaU(self, kF_1, kF_2, q, Q):
        """
        Evaluates \theta( kF_1 - \abs(Q_vec/2 + q_vec) ) \times
        \theta( kF_2 - \abs(Q_vec/2 - q_vec) ) for given q and Q. This function
        appears in the I term and the \delta U term.
        ...
        -------
        theta_deltaU : float
            \theta function evaluated at q and Q [unitless].

        """

        # Case 1: 2q+Q < 2kF_1 and 2q+Q < 2kF_2
        if 2*q+Q <= 2*kF_1 and 2*q+Q <= 2*kF_2:
            theta_1_deltaU = 1
                
        # Case 2: 2q+Q > 2kF_1 and 2q+Q > 2kF_2 and 4q^2+Q^2 < 2(kF_1^2+kF_2^2)
        elif 2*q+Q > 2*kF_1 and 2*q+Q > 2*kF_2 and \
             4*q**2 + Q**2 <= 2*(kF_1**2 + kF_2**2):
            theta_1_deltaU = ( 2*(kF_1**2 + kF_2**2) - 4*q**2 - Q**2 ) \
                             / (4*q*Q)
                            
        # Case 3: 2q+Q < 2kF_2 and -4 < (4q^2 - 4kF_1^2 + Q^2)/(qQ) < 4
        elif Q != 0 and 2*q+Q <= 2*kF_2 and \
             -4 < (4*q**2-4*kF_1**2+Q**2)/(q*Q) <= 4:
            theta_1_deltaU = ( 4*kF_1**2 - (Q-2*q)**2 ) / (8*q*Q)
                
        # Case 4: 2q+Q < 2kF_1 and -4 < (4q^2 - 4kF_2^2 + Q^2)/(qQ) < 4
        elif Q != 0 and 2*q+Q <= 2*kF_1 and \
             -4 < (4*q**2-4*kF_2**2+Q**2)/(q*Q) <= 4:
            theta_1_deltaU = ( 4*kF_2**2 - (Q-2*q)**2 ) / (8*q*Q)
                
        # Otherwise, F(Q,q) = 0
        else:
            theta_1_deltaU = 0
            
        return theta_1_deltaU
    
    
    def theta_deltaU2(self, kF_1, kF_2, Q, k_array, ntot_k):
        """
        Evaluates \theta( kF_1 - \abs(Q_vec/2 + k_vec) ) \times
        \theta( kF_2 - \abs(Q_vec/2 - k_vec) ) for every momentum k. This
        function appears in the \delta U \delta U^\dagger term.
        ...
        Q : float
            C.o.M. momentum [fm^-1].
        k_array : 1-D ndarray
            Momentum values [fm^-1] of \int dk k^2 integral in \delta U^2 term.
        ntot_k : int
        ...
        for i, k in enumerate(k_array):
            ...
            # Case 1: 2k+Q < 2kF_1 and 2k+Q < 2kF_2
            if 2*k+Q <= 2*kF_1 and 2*k+Q <= 2*kF_2:
                theta_deltaU2[i] = 1
                
            # Case 2: 2k+Q > 2kF_1 and 2k+Q > 2kF_2 and 
            # 4k^2+Q^2 < 2(kF_1^2+kF_2^2)
            elif 2*k+Q > 2*kF_1 and 2*k+Q > 2*kF_2 and \
                 4*k**2 + Q**2 <= 2*(kF_1**2 + kF_2**2):
                theta_deltaU2[i] = ( 2*(kF_1**2 + kF_2**2) - 4*k**2 - Q**2 ) \
                                    / (4*k*Q)
                            
            # Case 3: 2k+Q < 2kF_2 and -4 < (4k^2 - 4kF_1^2 + Q^2)/(kQ) < 4
            elif Q !=0 and 2*k+Q <= 2*kF_2 and \
                 -4 < (4*k**2-4*kF_1**2+Q**2)/(k*Q) <= 4:
                theta_deltaU2[i] = ( 4*kF_1**2 - (Q-2*k)**2 ) / (8*k*Q)
                
            # Case 4: 2k+Q < 2kF_1 and -4 < (4k^2 - 4kF_2^2 + Q^2)/(kQ) < 4
            elif Q !=0 and 2*k+Q <= 2*kF_1 and \
                 -4 < (4*k**2-4*kF_2**2+Q**2)/(k*Q) <= 4:
                theta_deltaU2[i] = ( 4*kF_2**2 - (Q-2*k)**2 ) / (8*k*Q)
    ...
    def n_1(self, q, Q, kF_1, kF_2):
        """
        Evaluates both the \theta functions in U n(q) U^\dagger ~ I n(q) I
        which gives \theta( kF_1(r) - |Q/2+q| ) \theta( kF_2(r) - |Q/2-q| ).
        The full first term is given by
            2 \int d3r \theta( kF_1(r) - |Q/2+q| ) \times
            \int d3r' \theta( kF_2(r) - |Q/2-q| ),
        ...
        -------
        output : float
            Momentum distribution from I term before integrations over
            \int dr r^2 and \int dr' r'^2.
    ...
    def n_deltaU(self, q, Q, kF_1, kF_2, distribution='pn'):
        """
    ...
    def n_deltaU2(self, q, Q, kF_1, kF_2, distribution='pn'):
        """
    ...
    def n_lambda(self, q_array, Q, r_array, rho_1_array,
