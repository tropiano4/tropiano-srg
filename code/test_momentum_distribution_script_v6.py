...
def delta_U_quantum_numbers_OLD(tau, occ_states, cg_table, channels):
  """Returns a list of every combination of quantum numbers in the \delta U
  linear terms that gives a non-zero product of Clebsch-Gordan coefficients.
  """
  ...
  for channel in channels:
    ...
    for M_T in np.arange(-T, T+1):
      ...
      for alpha in occ_states:
        ...
        del_alpha = int(alpha.m_t == tau)
        
        # TESTING
        # del_l_alpha = int(alpha.l == 0)
        del_l_alpha = int(alpha.l == 1)

        # Occupied single-particle state \beta
        for beta in occ_states:
                    
          # Kronecker \delta function \delta_{\tau_\beta \tau}
          del_beta = int(beta.m_t == tau)
          
          # TESTING
          # del_l_beta = int(beta.l == 0)
          del_l_beta = int(beta.l == 1)
                    
          # \tau_\alpha \tau_\beta T M_T CG
          if alpha.m_t + beta.m_t == M_T:
            isospin_cg = cg_table[(1/2, alpha.m_t, 1/2, beta.m_t, T, M_T)]
          else:
            isospin_cg = 0
...
def delta_U2_quantum_numbers_OLD(tau, occ_states, cg_table, channels):
  """Returns a list of every combination of quantum numbers in the \delta U^2
  term that gives a non-zero product of Clebsch-Gordan coefficients.
  """
          ...
          for M_Tp in np.arange(-Tp, Tp+1):
            ...
            for alpha in occ_states:
              ...
              for beta in occ_states:
                ...
                # \tau_\alpha \tau_\beta T M_T CG
                if alpha.m_t + beta.m_t == M_T:
                  isospin_ab_cg = cg_table[(1/2, alpha.m_t, 1/2, beta.m_t, T,
                                            M_T)]
                else:
                  isospin_ab_cg = 0
                                    
                # \tau_\alpha \tau_\beta T' M_T' CG
                if alpha.m_t + beta.m_t == M_Tp:
                  isospin_abp_cg = cg_table[(1/2, alpha.m_t, 1/2, beta.m_t, Tp,
                                             M_Tp)]
                else:
                  isospin_abp_cg = 0
