...
def phase_shifts(e_array, V_matrix, k_array, k_weights):
    ...
    for i, ie in enumerate(e_array):
        ...
        D_last = -2.0 / np.pi * k0 ** 2 * (
                    np.sum(k_weights / (k_array ** 2 - k0 ** 2))
...
def coupled_channel_phase_shifts(
    ...
    for i, ie in enumerate(e_array):
        ...
        # Stapp convention
            ...
            eps_bar = 0.5 * np.arcsin(
                np.sin(2.0 * eps) * np.sin(delta_a - delta_b))
            delta_bar_a = 0.5 * (
                    delta_a + delta_b
    ...
    # This is a mtot x 3 dimensional array where mtot is the length of e_array
    return phase_shifts
