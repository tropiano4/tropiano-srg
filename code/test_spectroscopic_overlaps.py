...
class DeltaUIntegrand:
    ...
    def __init__(
            self, q, alpha, woods_saxon, delta_U_matrix_element, spin_configurations,
...
# TODO: Check real/complex parts!
def compute_I_term(q_array, alpha, woods_saxon):
    """Compute the I term."""
        
    # I_array = np.zeros_like(q_array, dtype=complex)
    # theta_array = np.zeros_like(q_array)
    # phi_array = np.zeros_like(q_array)
    theta_array, theta_weights = gaussian_quadrature_mesh(np.pi, 15)
    ...
    _, dtheta_grid, dphi_grid = np.meshgrid(
        q_array, np.sin(theta_array) * theta_weights, phi_weights, indexing='ij'
    )
    ...
    for sigma in np.array([1/2, -1/2]):
        ...
        for tau in np.array([1/2, -1/2]):
            ...
            psi_alpha_grid = woods_saxon.psi(alpha, q_grid, theta_grid,
                                             phi_grid, sigma, tau)

            I_grid += psi_alpha_grid
...
def compute_delta_U_dagger_term(
        q_array, alpha, woods_saxon, delta_U_matrix_element, neval
):
    ...
    for i, q in enumerate(q_array):
        ...
        integrand = DeltaUIntegrand(
            q, alpha, woods_saxon, delta_U_matrix_element, spin_configurations,
            isospin_configurations
        )
...
def compute_overlap(
    ...
    # Compute the I term
    I_array = compute_I_term(q_array, alpha, woods_saxon)
...
def compute_normalization(q_array, q_weights, overlap_array):
    ...
    return np.sum(q_weights * q_array ** 2 * overlap_array ** 2)
...
if __name__ == '__main__':
    ...
    # TODO: This won't work unless you integrate over angles of q
    # Quantum state
    alpha = SingleParticleState(1, 0, 1/2, 1/2, 1/2)  # Proton 1s_1/2
    # alpha = SingleParticleState(1, 1, 3/2, 1/2, 1/2)  # Proton 1p_3/2
    ...
    q_array, q_weights, overlap_array, delta_U_errors = compute_overlap(
        nucleus_name, Z, N, alpha, kvnn, lamb, channels, neval=neval,
        kvnn_hard=kvnn_hard, lambda_m=lambda_m, parametrization=prm, ipm=True,
        save=False
    )