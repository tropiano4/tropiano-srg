...
but integrates over angles of q.
...
def compute_I_term(q_array, tau, occ_states, cg_table, phi_functions):
    ...
    theta_array, theta_weights = gaussian_quadrature_mesh(np.pi, 25)
    phi_array, phi_weights = gaussian_quadrature_mesh(2*np.pi, 25)
    
    q_grid, theta_grid, phi_grid = np.meshgrid(q_array, theta_array, phi_array,
                                               indexing='ij')
    _, dtheta_grid, dphi_grid = np.meshgrid(
        q_array, np.sin(theta_array) * theta_weights, phi_weights,
        indexing='ij'
    )
    I_grid = np.zeros_like(q_grid)
    ...
    for sigma in np.array([1/2, -1/2]):
        ...
        for alpha in occ_states:
            ...
            # Single-particle wave function at (q, \theta, \phi)
            psi_alpha_grid = psi(
                alpha.n, alpha.l, alpha.j, alpha.m_j, alpha.m_t, q_grid,
                theta_grid, phi_grid, sigma, tau, cg_table, phi_functions
            )
 
            # I term with angular dependence
            I_grid += np.abs(psi_alpha_grid) ** 2
            
    # Integrate over angles
    I_array = np.sum(I_grid * dtheta_grid * dphi_grid, axis=(1,2))
...
def delta_U_term_integrand(
    ...
    # Momenta q
    theta_q, phi_q = x_array[:2]
    q_vector = build_vector(q, theta_q, phi_q)

    # Relative momenta k
    k, theta_k, phi_k = x_array[2:5]
    k_vector = build_vector(k, theta_k, phi_k)
        
    # C.o.M. momenta K
    K, theta_K, phi_K = x_array[5:8]
    K_vector = build_vector(K, theta_K, phi_K)
    ...
    jacobian = (np.sin(theta_q) * k ** 2 * np.sin(theta_k) * K ** 2 
                * np.sin(theta_K))
    
    # Index for quantum numbers
    index = np.floor(x_array[8] * delU_Ntot).astype(int)
...
def compute_delta_U_term(
    ...
    integ = vegas.Integrator([theta_limits, phi_limits,
    ...
    for i, q in enumerate(q_array):
        ...
        integrand = functools.partial(
            delta_U_term_integrand, q, tau, delta_U_quantum_numbers,
            cg_table, phi_functions, delta_U_functions,
            delta_U_dagger_functions, delU_Ntot
        )
        ...
        mins = (t1-t0) / 60
        print(f"{percent:.2f} percent done after {mins:.3f} minutes.")
...
def delta_U2_term_integrand(
    ...
    # Relative momenta k'
    kp, theta_kp, phi_kp = x_array[5:8]
    kp_vector = build_vector(kp, theta_kp, phi_kp)
        
    # C.o.M. momenta K
    K, theta_K, phi_K = x_array[8:11]
    K_vector = build_vector(K, theta_K, phi_K)
    ...
    # Calculate the Jacobian determinant
    jacobian = (np.sin(theta_q) * k ** 2 * np.sin(theta_k) * kp ** 2
                * np.sin(theta_kp) * K ** 2 * np.sin(theta_K))
    
    # Index for quantum numbers
    index = np.floor(x_array[11] * delU2_Ntot).astype(int)
    # Samples of quantum number sets
...
def compute_delta_U2_term(
    ...
    for i, q in enumerate(q_array):
        ...
        # Set the integrand function
        integrand = functools.partial(
            delta_U2_term_integrand, q, tau, delta_U2_quantum_numbers,
            cg_table, phi_functions, delta_U_functions,
            delta_U_dagger_functions, delU2_Ntot
        )
...
def compute_momentum_distribution(
    ...
    # Include \delta U, \delta U^\dagger, and \delta U \delta U^\dagger
        ...
        delta_U_array, delta_U_errors = compute_delta_U_term(
            q_array, tau, cg_table, channels, phi_functions,
...
def compute_normalization(q_array, q_weights, n_array):
    ...
    return np.sum(q_weights * q_array**2 * n_array)
