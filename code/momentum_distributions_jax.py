...
from scripts.integration import (
    momentum_mesh, gaussian_quadrature_mesh, unattach_weights_from_matrix
)
...
class WoodsSaxon:
    ...
    def set_mapping(self, occupied_states):
        ...
        self.occupied_states_array = jnp.array(occupied_states)
        n_max = jnp.amax(self.occupied_states_array[:, 0])
        l_max = jnp.amax(self.occupied_states_array[:, 1])
        self.j_max = jnp.amax(self.occupied_states_array[:, 2])
...
# TODO: UPDATE k AND k' DEPENDENCIES (NEED TO BE INTERPRETED AS ARRAYS)
...
class DeltaUIntegrand:
    ...
    def __call__(self, q, x_array):
        ...
        return jnp.sum(
            self.vmap_isospin(q, x_array, self.isospin_configurations)
        )
    
    @partial(jit, static_argnums=(0,))
    def vmap_isospin(self, q, x_array, isospin):
        """vmap of evaluate method with shape (N_isospin,)."""

        return vmap(self.evaluate, in_axes=(None, None, 0))(q, x_array, isospin)
    
    @partial(jit, static_argnums=(0,))
    def evaluate(self, q, x_array, isospin):
        ...
        # Isospin projections \tau', \tau_1, and \tau_2
        taup, tau_1, tau_2 = isospin
        
        # s.p. states \alpha, \beta < F
        n_alpha, l_alpha, j_alpha, m_j_alpha, m_t_alpha = (
            self.get_occupied_state(x_array[7]))
        n_beta, l_beta, j_beta, m_j_beta, m_t_beta = (
            self.get_occupied_state(x_array[8]))
        ...
        return integrand.real * self.N_spin * self.N_occupied_states ** 2
    ...
    def get_spin_configuration(self, x):
        ...
        index = jnp.floor(x * self.N_spin).astype(int)
        return self.spin_configurations[index]
    
    @partial(jit, static_argnums=(0,))
    def get_occupied_state(self, x):
        """Given a number between 0 and 1, return an occupied Woods-Saxon
        orbital.
        """
        
        index = jnp.floor(x * self.N_occupied_states).astype(int)
        return self.occupied_states[index]
...
class MomentumDistribution:
    ...
    def __init__(
        self, nucleus_name: str, Z: int, N: int, kvnn: int, lamb: float,
        kmax=15.0, kmid=3.0, ntot=120, generator='Wegner'
    ):
        ...
        # Set \delta U plane-wave matrix element function
        ...
        K_array, K_weights = gaussian_quadrature_mesh(3.0, 60)
        theta_array, theta_weights = gaussian_quadrature_mesh(np.pi, 13)
        phi_array, phi_weights = gaussian_quadrature_mesh(2*np.pi, 17)
        
        # Set meshgrids as instance attributes for integration
        (K_grid, theta_K_grid, phi_K_grid, k_grid, theta_k_grid,
         phi_k_grid) = jnp.meshgrid(
             K_array, theta_array, phi_array, k_array, theta_array, phi_array,
             indexing='ij'
        )
        (dK_grid, dtheta_K_grid, dphi_K_grid, dk_grid, dtheta_k_grid,
         dphi_k_grid) = jnp.meshgrid(
             K_weights, theta_weights, phi_weights, k_weights, theta_weights,
             phi_weights, indexing='ij'   
        )
             
        # Evaluate Jacobian for 6-D integrand (K_vector, k_vector)
        self.jacobian = (
            K_grid ** 2 * dK_grid * jnp.sin(theta_K_grid) * dtheta_K_grid
            * dphi_K_grid * k_grid ** 2 * dk_grid * jnp.sin(theta_k_grid)
            * dphi_k_grid
        )
    
    def __call__(
            self, tau, qmax=10.0, qmid=3.0, ntotq=100, nmodq=60,
        ...
        # Compute the I term (independent-particle model)
        I_array = self.I_term(tau, q_array)
        print("Done with the I term.")
        
        # Compute the \delta U + \delta U^\dagger terms
        delta_U_array = self.delta_U_term(tau, q_array)
        print("Done with the \delta U + \delta U^\dagger terms.")
    ...
    def I_term(self):
        # For now python loop over \alpha and spins!
        pass
    
    def delta_U_term(self):
        # Python loop over spins, isospins, \alpha, and \beta
        # integrating over d3K of method delta_U_K
        pass
    
    def delta_U2_term(self):
        # Python loop over spins, isospins, \alpha, and \beta
        # integrating over d3K of method delta_U2_K
        pass
    ...
    @partial(jit, static_argnums=(0,))
    def delta_U2_Kk(self):
        pass
    
    @partial(jit, static_argnums=(0,))
    def delta_U2_Kkp(self):
        pass
...
class MomentumDistribution:
    ...
    def delU_term(self, tau, q_array, neval):
        ...
        phi_array, phi_weights = gaussian_quadrature_mesh(2*np.pi, 21)
