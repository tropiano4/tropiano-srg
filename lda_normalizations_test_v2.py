...
#   Testing normalization and contributions of \delta U, etc. to pair momentun
#   distributions.
...
x_label_size = 16
if case == 'pp':
    y_label = r'$n^{pp}_A(q,0)$' + ' [fm' + r'$^3$' + ']'
else:
    y_label = r'$n^{pn}_A(q,0)$' + ' [fm' + r'$^3$' + ']'
y_label_size = 16
legend_size = 13
...
for lamb in lambda_array:
    ...
    for nucleus in nuclei_list:
        ...
        ax.plot(q_array, n_1_array*fac, color='xkcd:blue', label='1',
                linestyle='dotted', linewidth=curve_width)
        ax.plot(q_array, abs(n_delU_array)*fac, color='xkcd:green',
                label=r'$|\delta U|$', linestyle='dashed', linewidth=curve_width)
        ax.plot(q_array, n_delU2_array*fac, color='xkcd:red',
                linestyle='dashdot', label=r'$\delta U \delta U^{\dagger}$',
                linewidth=curve_width)
        
        # Add AV18 data with error bars
        if nucleus_name in ('C12'):
            av18_data = np.loadtxt(data_directory+'/'+'AV18_%s_pmd.txt'
                                   % nucleus_name)
            ...
            if case == 'pp':
                ax.errorbar(q_array_av18, n_pp_array_av18,
                            yerr=pp_error_bars_array_av18, color='xkcd:black',
                            label='AV18', linestyle='', marker='.')
            else:
                ax.errorbar(q_array_av18, n_pn_array_av18,
                            yerr=pn_error_bars_array_av18, color='xkcd:black',
                            label='AV18', linestyle='', marker='.')
